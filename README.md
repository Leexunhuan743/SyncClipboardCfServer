...
协议面与官方服务端逐条对齐（差异与依据登记在 `docs/protocol.md` §10）；下表只列**会实际影响使用**的那几条：

| 现象 | 与官方服务端的差别 | 影响与对策 |
|---|---|---|
| 历史不会按时间自动过期 | 保留期默认 **0 = 不限制**（官方 3.3.0 起同口径；官方旧版默认 7 天） | 默认只按 1000 条上限回收，且**收藏/置顶不参与裁剪**。需要按时间回收请设 `HISTORY_RETENTION_MINUTES` |
| 删除后仍占用存储 | 删除是**软删**：记录进回收站且**保留数据文件**，可连数据恢复；30 天后硬删 | 官方软删即删数据（图片/文件删了拿不回来）。代价是多占 ≤30 天空间；「彻底删除 / 清空回收站」可立刻释放 |
| 单个请求体超过 48 MiB 被拒（413） | 官方无请求体上限（Kestrel `int.MaxValue`） | 客户端默认单文件 20 MB，不受影响；确实需要更大的文件可把 `MAX_REQUEST_BODY_BYTES` 调到 64 MiB |
| 文件夹同步 / 大文本有额外封顶 | Group：解压总量 64 MiB、条目 1000 项、**单条目 24 MiB**、膨胀比 100:1；内联文本 1 MiB | 官方没有这些上限。超限时正常上传返回 422、设置当前剪贴板返回 400；把大内容拆小或改用文件同步 |
| 升级前入库的记录不做完整性校验 | `/data` 不回带 `X-SyncClipboard-Transfer-Data-Hash` | 客户端据此**跳过**该校验（官方靠惰性回填，本实现不回填）。升级后新上传的记录都带 |
| 浏览器访问不存在的路径可能触发限速 | 未认证返回 401 并计入失败计数 | 同一 IP 15 分钟内失败 10 次会锁 15 分钟（响应带 `Retry-After`，到期自动恢复）；正常使用不受影响 |
| 不能导入官方服务端的 `history.db` | 表结构与时间列类型不同（INTEGER epoch 毫秒 vs TEXT ISO8601） | 两者不共读同一份数据；换服务端后请让客户端重新同步历史 |
| 畸形压缩包的处理口径不同 | 重复条目：官方双重计入 hash/体积，本实现去重取首个；`.` / `..` 段：官方按平台归一化接受，本实现拒绝 | 官方客户端生成的 zip 不会出现这两种形态（不可达）；第三方工具造的畸形包可能被拒 |

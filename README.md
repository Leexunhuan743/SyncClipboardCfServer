...
界面支持键盘快捷键（`/` 搜索、`r` 刷新、`t` 切主题、`n`/`p` 翻页；在某一行的任何控件上按 `↑`/`↓` 换行，`v` 预览、`c` 复制、`d` 下载、`s` 收藏、`i` 置顶、`Delete` 移到回收站，`Shift+↑/↓` 连续选择；预览框里 `c`/`d`/`e`，编辑时 `Ctrl+Enter` 保存、`Esc` 返回）—— 各按钮的悬浮提示里都写着对应的键，按 `?` 可以随时调出完整列表。

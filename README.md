...
协议面与官方服务端逐条对齐；下表只列**会实际影响使用**的那几条，两侧行为逐列对写
（每条的上游源码依据与登记都在 `docs/protocol.md` §10）：

| 方面 | 官方服务端 | 本项目（cfserver） | 影响与对策 |
|---|---|---|---|
| 历史过期 | 保留期默认 **0 = 不限制**（3.3.0 起；旧版默认 7 天 = 10080 分钟）；条数上限默认 1000，收藏/置顶不参与回收 | 同口径：保留期默认 0、上限 1000、收藏/置顶豁免 | 默认只有条数上限在回收。要按时间回收请设 `HISTORY_RETENTION_MINUTES`（如 `10080` = 7 天） |
| 删除的语义 | 软删时**立即删掉数据文件**，回收站只剩记录 → 图片/文件删了拿不回来 | 软删**保留数据文件**，30 天后由清理任务硬删；回收站里可连数据恢复（含已删记录的 `/data` 仍可下载） | 代价是多占 ≤30 天 R2 空间；点「彻底删除 / 清空回收站」可立刻释放 |
| 单个请求体上限 | 无上限（Kestrel `MaxRequestBodySize = int.MaxValue`） | 默认 **48 MiB**、可调到 64 MiB，超限 413（isolate 只有 128 MiB 且并发共享） | 客户端默认单文件 20 MB，不受影响；确实要同步更大文件时调 `MAX_REQUEST_BODY_BYTES` |
| 文件夹 / 内联文本的封顶 | 无封顶：Group 全量解压、内联文本长度不限 | Group：解压总量 64 MiB、条目 1000 项、**单条目 24 MiB**、膨胀比 100:1；内联文本 1 MiB | 超限时 `POST /api/history` 返回 422、设置当前剪贴板返回 400；官方客户端默认 20 MB 内基本不可达 |
| 升级前记录的完整性哈希 | `PrepareTransferData` 对旧记录**惰性回填**，之后 `/data` 一律回带 `X-SyncClipboard-Transfer-Data-Hash` | **不回填**（重算要把整个对象读进内存，代价不成比例）⇒ 迁移前入库的记录不带该响应头 | 客户端对该记录**跳过**这层校验（新上传的记录都带）；只影响升级前的老记录 |
| 未知路径的鉴权 | 路由不匹配即 404，**不碰凭据**（也无限速） | 全局 Basic 认证先行 ⇒ 未带凭据返回 401，且**计入认证失败计数** | 同一 IP 15 分钟内失败 10 次会临时封锁 15 分钟（响应带 `Retry-After`，到期自动恢复）；正常使用不受影响 |
| 数据迁移 | 本地 SQLite 文件 `history.db`，时间列为 TEXT（ISO8601） | D1（同一 SQLite 引擎），时间列为 INTEGER（epoch 毫秒） | 两套表结构不通用 ⇒ **不能把 `history.db` 直接导入**；换服务端后让客户端重新同步历史 |
| 畸形压缩包的口径 | 重复条目**双重计入** hash 与体积；`.`/`..` 段按平台归一化后接受（Windows / Linux 行为不同） | 重复条目取首个并去重；`.`/`..` 段一律拒绝（跨平台一致） | 官方客户端生成的 zip 不含这两种形态（不可达）；第三方工具造的畸形包可能被拒 |
| 错误响应体 | `BadRequest()` 空体；422 为 ProblemDetails JSON | 400 类返回纯文本消息；422 返回**与官方同构**的 ProblemDetails JSON（`status`/`title`/`code`/`detail`） | 官方客户端只判状态码；第三方若解析 400 的错误体会拿到文本而非空体 |

...
// 汇总行是「eyebrow + 值」两个子节点；保存设置后只换值那一半，不必重建整行。
function setFieldValue(field, text) {
  const value = field.querySelector('span:not(.eyebrow)');
...
export function createInfo({ onCopyText, onClearAll, getClockOffsetMs, getLastChangeMs, getPushState }) {
  ...
  function clockRow() {
    ...
    if (offset === null || !Number.isFinite(offset)) return row('服务端时间', '尚未观测到（下一次轮询后显示）');
  ...
  function cleanupSection(cleanup) {
    ...
    return section('清理任务', [
      row('最近一次运行', cleanup?.lastRunAt ? formatAbsolute(cleanup.lastRunAt) : '从未运行（Cron 未触发过）'),
      row('上次失败', cleanup?.lastError ?? '无'),
      row(
        '续跑游标',
  ...
  function retentionSection(retention, onSaved) {
    ...
    const save = el(
      'button',
      {
        ...
        onclick: async () => {
          ...
          if (Object.values(patch).some((value) => value !== null && (!Number.isSafeInteger(value) || value < 0))) {
            source.textContent = '只能填非负整数（分钟 / 条数）；留空表示回落到部署时的环境变量。';
            return;
          }
    ...
    return section('保留策略', [
      el('span', {
        class: 'note',
        text: '留空 = 用部署时的环境变量；0 = 关闭对应阶段（不再按时间/条数清理）。改动立即对下一轮清理生效。',
      }),
      el('div', { class: 'panel__form' }, [
        el('label', { class: 'field-inline' }, [el('span', { class: 'eyebrow', text: '保留分钟' }), minutes]),
        el('label', { class: 'field-inline' }, [el('span', { class: 'eyebrow', text: '条数上限' }), maxCount]),
        save,
      ]),
      source,
    ]);
  ...
  return {
    open(info) {
      ...
      const retentionRow = row('保留策略', retentionText(retention));
      ...
      body.replaceChildren(
        ...
        row('服务端支持的传输', transports || '—'),
        row('实时推送', PUSH_LABELS[getPushState?.() ?? 'offline'] ?? '未知'),
        retentionRow,
        row(
          '数据体积',
        ...
        clockRow(),
        row('最近一次变更', Number.isFinite(lastChangeMs) ? formatAbsolute(isoOf(lastChangeMs)) : '尚未观测到'),
        cleanupSection(info.cleanup),
        retentionSection(retention, () => setFieldValue(retentionRow, retentionText(retention))),
        integritySection(),
        dangerSection(),
      );

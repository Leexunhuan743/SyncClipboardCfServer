...
//
// ⚠️ 行数必须**尽量接近真实页大小**（2026-09-16 实测）：骨架行高与真实行同高（2026-09-19 起由 `board-v2.css` 真正保证
// —— 表格档是 `calc(var(--row-h) + var(--sp-3) + 1px)`，卡片档（≤720px）另有一套按卡片盒模型推出来的公式，
// 两档都有实测数值与探针判据钉着；此前表格档写死 `var(--row-h)`，每行差 13px、50 行满页差 650px，
// 而三处注释都声称相等。卡片档修前是 77 vs 125/135px，2026-09-19 修齐 —— 见 docs/progress.md §94 第 13 行），
...
/** 建 N 行骨架。行高与真实行同高（表格档 `calc(var(--row-h) + var(--sp-3) + 1px)`、卡片档另有一套，两者的推导与实测都在 `board-v2.css`），所以**每行**落位不跳；

...
export function createBoard(handlers) {
  ...
  function update(spec) {
    ...
    if (state === 'loading') {
      ...
      // 行高等式（2026-09-19 修，审计 §10.1）：真实行 = calc(var(--row-h) + var(--sp-3) + 1px)，
      // 与 .ghost 的高度同源；实测 1440×900 下宽松/紧凑两档的差都是 0px；窄屏卡片档同理
      // （2026-09-19 起 .ghost 在 ≤720px 也有按卡片盒模型推出来的高度，见 board-v2.css）。

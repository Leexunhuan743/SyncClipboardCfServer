...
// ===== 挂载点：页面在 `/ui_v1/`、接口在 `/ui/api/`，两者不是同一个前缀 =====
//
// 这个区别必须有唯一的字面量（下面这个 `API_BASE`）。教训有据可查：2026-09-15 把 V1 存档进
// 它自己的目录时，前缀的批量改写**把接口前缀也一起改了** —— 接口请求被
// 打到界面自己的挂载点下面（界面前缀后面接 `api/`），而服务端从不提供那个命名空间
// （`src/index.ts` 把界面那一面整体当静态资源交给 `env.ASSETS`）。症状极具迷惑性：
...
export const api = {
  ...
  // 批量写：`update` 是要应用到每一条的字段集合（收藏/置顶/软删/恢复共用这一条路径）。

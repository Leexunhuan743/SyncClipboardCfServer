...
// 自己的 `js/messages.js` 与 V2 的同名文件逐字一致、令牌不空转、可点控件的按下反馈清单。
// 这条分工是刻意的 —— 两边都用同一种判据（剥注释后按页解析 import 闭包），只是扫描目标不同。
// （两处曾与事实不符的说法已在 2026-09-18 修掉：① 早先写"V1 已冻结、不受本套件约束"；
//  ② 后来写"V1 的**文案表与 V2 共用同一份**"——V1 是自包含的，它有自己的一份 `messages.js`，
//  靠 `ui-guard` 的对等守卫保持逐字一致，而不是共享文件。）
//

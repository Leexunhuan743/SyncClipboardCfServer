...
- **F-1 提到的语义不一致**：V2 清空筛选与空状态里的「清除筛选」对 `keepView` 取值相反
  （前者回活跃列表、后者留在回收站）。本轮只修了**指向错的注释**，没有改行为——
  要统一需要先定"哪种才是对的"，属产品决策（`docs/AUDIT-commit-9b4cdca.md` §P2 记的就是这件事）。
  ⚠️ **已定案（2026-09-20，ADR D19 / `progress.md` §103.10）**：两处统一为**回活跃列表** ——
  V2 空状态那枚不再传 `keepView`、`boot.js` 的形参删除，`states.mjs` 里钉着旧行为的断言同批改掉。
...
| V2 `board-v2.css`（分组标题量级） | `50×125 + 49×8 = 6642`，实测 6683.05，差 41px | ✅ 6642 / 差 **41.05** | 差值归因于分组标题 `.daymark` + 它自己的间距 —— **归因是解释性的，未单独实测**（⚠️ **2026-09-20 已用构造数据集实测**：注入 50 张同形卡片 + 1 条分组标题 ⇒ `49×125 + 1×124 + 34.05 + 50×8 = 6683.05`，**逐位复现**当年读数（差 0）；差 41.05 = 34.05 + 8 − 1。见 `progress.md` §103.12） |

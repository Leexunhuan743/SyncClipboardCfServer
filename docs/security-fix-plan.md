...
- **完整报告**：`.audits/cfserver-audit-003/report.md`（含每条 Finding 的证据 id、上游对照、过程披露）。**该目录只存在于审计时那台机器的本地工作区**（`.gitignore:14` 排除、不入库），仓库检出里没有它——引用它的命令见 §五 的边界说明
...
> `npm test` **全部 22 个套件通过**（用例数见命令输出）；F5/F6 另有"翻转探针"证据（修前 500/落库 → 修后 200/400）。

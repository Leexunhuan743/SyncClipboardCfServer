...
- **来源**：`cfserver-audit-003`（交叉验证安全审计）。该审计**绑定的是一个重排前存在、现已不存在的提交**（`52bfb333…`）：本轮把 91 条提交按主题重排为 12 条，旧 SHA 均不再指向当前历史；审计账目保留在`.audits/cfserver-audit-003/`（不在版本库内），其 `state.json` 内仍记录重排前的快照 SHA。
...
> `npm test` **全部 18 个套件通过**（用例数见命令输出）；F5/F6 另有"翻转探针"证据（修前 500/落库 → 修后 200/400）。

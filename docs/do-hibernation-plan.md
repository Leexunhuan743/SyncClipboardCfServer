...
- **状态（2026-09-25，两轮之后）：仍不能判定 ⇒ P3 的「必要性」是未回答的。** D（`HibernatingSse`）
  与 C 两臂**第二轮补测仍未测出**（D 保持 60.2 s 后 `curl 56`、指标全 0 且 `requests=0`；C 保持 60.9 s 后 WS `1006`、
  指标不全）；根因是**客户端路径 ~60 s 静默连接硬切** + **新命名空间 Analytics 落地慢**（§4.1 的两条环境事实），
  **不是**指标问题 ⇒ 重跑配方见 §4.1。

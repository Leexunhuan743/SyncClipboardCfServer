...
| 动效 | 事件 | 时长 |
|---|---|---|
| 行入场错峰 | **仅首屏** | 每行 28ms、上限 10 行。⚠️ **V1 已于 2026-09-22 整条移除**（用户要求：逐行冒出来读起来是"慢"）⇒ 这是两版**有意不同**的一处：V2 保留（开发测试版，允许后续破坏性重构），记在 `docs/progress.md` §155 |
...
**占位与真实必须同高**（三条判据都在 `test/manual/probe.mjs`）：行骨架 = 真实行的**众数**行高、
骨架之间 = 卡片之间的间距、**概览带的值占位 = 真实值那一行的行盒**。第三条是 2026-09-22 补的：
`.overview__ghost` 原先（`0.7em` = 12px）比真实值矮 6px，数据落地时每格长高 6px；加上窄屏档
（≤720）**唯一**显示的趋势图从 0 长到 26px ⇒ 概览带整体高 32px，把 omnibox 与看板推下去，
窄屏首屏 CLS **0.1217**（宽屏只有 0.0026 —— 所以那条 CLS 判据在宽屏**看不见**它，才需要按几何钉）。
修法与实测见 `docs/progress.md` §154。
...
| 批量条 | 出现、`position: fixed`、**不遮挡行**；活跃视图禁用「恢复」、回收站视图禁用「移动到回收站」 |
...
- ~~单条"不可恢复"与批量"恢复"的判据写法不一致（`hasData` vs `hasData !== true`）~~ ——
  **2026-09-22 已修**：单条那处（`rowops.js` / `menus.js`）随 ADR D29 改成"回收站里所有记录都能恢复"，
  批量那处（`ui/batchbar.js`）同日一并改成 `set(buttons.get('restore'), deleted)`。两处现在同一条判据。
...
**2026-09-22（ADR D29）起"不可恢复"这一档没有了**：软删不再清数据，带数据文件的记录恢复时会连数据
一起回来（服务端那条上游守卫已去掉），故主操作「恢复到历史记录」**对回收站里所有记录都可用** ——
`rowops.js` / `menus.js` 同日不再按 `hasData` 禁用（`renderRowOps` 仍收同一个 `onRestore`，
3 个调用点与 `boot.js` 的 `restoreItem` 都没变）。

**验证**：`states.mjs` 进 `?deleted=1`，断言第一行的主操作图标是 `undo`、且**没有被禁用**
（D29 之后钉的是"不禁用"—— 此前那条"被禁用时必须说明原因"已成空转）。

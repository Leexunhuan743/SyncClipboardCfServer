...
│ ⬢ 剪贴板历史      ⌕    ⓘ   ⏻ │  顶栏：搜索收成图标
├───────────────────────────────┤
│ 1009 条 · 已同步 2 分钟前     │  概览带压缩成一行（趋势图隐藏）
│ ▁▂▅▃▇▂▁▃▅          20 KB    │
├───────────────────────────────┤
│ ⌕ 搜索…                       │  搜索（点击后全屏聚焦态）
├───────────────────────────────┤
...
| 令牌 | 值 | 用途 |
|---|---|---|
| `--fs-display` | `clamp(1.75rem, 1.4rem + 1.4vw, 2.25rem)` | 概览带主数字（V1 是 1.875rem，偏小） |
...
│       ├── state.js           状态容器（`patch` / `update`，刻意不提供订阅）
│       ├── filters.js         筛选状态 ⇄ URL
...
| **P5** | 文档收尾 + 守卫测试扩展 | ✅ | 20 套件 / 347 用例全绿（§12.4） |

**N4（`/ui/api/clients`）与 N5（`/ui/api/export`）未实施** —— 理由记在 §12.6，都是"收益不抵成本 /
属于独立立项"，不是遗漏。前端 `api.js` 里已经写好了 `clients()` 的调用封装并注明了失败语义
（`{clients:null}` 而非整体 5xx），将来接线时不必重编契约。
...
- `npm test` ✅ **20 个套件 / 347 个用例全通过**（含 `ui-contract` 10 条跨文件契约、`ui-guard` 8 条鉴权与开关、`docs` 7 条文档口径）
...
- `npm run check` ✅
- `npm test` ✅ **20 套件 / 347 用例全绿**（含被修复的 `query-filters` 8 条）
...
**本轮验证**：`npm run check` ✅；`npm test` ✅ **20 套件 / 358 用例**（+11 条纯逻辑单测）；

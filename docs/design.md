...
⚠️ **2026-09-26（合并前审查 F1）**：首批的行字节在取回前未知，原先退化成「按 500 条取」⇒ 500 条 × 4 KB 的形态
一次进内存约 2 MB（正是 `progress.md` §186.4 那条用例量到的）。现在**首批只取 5 条探路**（`FIRST_BATCH_ROWS`），
取回后按实测均值收窄 ⇒ 大行形态不再有一次性的 ~2 MB 越界。

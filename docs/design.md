...
⚠️ **2026-09-26（合并前审查 F1，第二轮改为更强形态）**：首批的行字节在取回前未知，原先退化成「按 500 条取」
⇒ 500 条 × 4 KB 一次进内存约 2 MB（`progress.md` §186.4 的用例量到的）；第一轮的修法「首批只取 5 条探路 +
按实测**均值**收窄」仍会被**异构行**骗过（前 5 条空文本 ⇒ 均值≈0 ⇒ 下一批按 500 条取，实测单轮
**6,553,600 字节 = 25 × 预算**，见 §188）。现在每批都先跑一条**只读**的 `length(Text)` 扫描
（`db.scanSoftDeleteCandidateBytes`，列顺序与删除语句的 `ORDER BY` 逐字一致）量出候选逐行字节，
再按前缀和决定取回几条 ⇒ **与行大小是否均匀无关**，`FIRST_BATCH_ROWS` 已删除。
单轮**仍可能**被平台终止（单条记录自带 1 MiB 内联文本时任何预算都挡不住**持续**超限），但被终止时**轮首心跳**
已把 `cleanup:lastRunAt` 落库、而**完成戳** `cleanup:lastCompletedAt` **只在轮尾写** ⇒ 「只有起点、没有完成戳」
在 `/ui/api/info` 与统计条上显示为 **「清理未完成」**（2026-09-26，审查 F2：判据从「lastError 是否为空」改成完成戳 ——
只看 lastError 会把被平台终止的轮次显示成「清理正常」）。见 §9 与 §13 的对应行。
...
| Free 计划的清理预算：**10 ms CPU 平均**（HTTP 与 Cron 同一档，平台另有 rollover CPU time）+ 1,000 次/调用「到 Cloudflare 服务」的子请求 | 中 | 子请求侧 800/轮 + 阶段保底 + 游标续跑；**CPU 侧自 2026-09-25 起有 256 KiB/轮/阶段的**行字节预算（软删两阶段按**候选行的只读字节扫描**硬约束，
硬删/孤儿仍按条数上限，条数上限不动）⇒ **双重截断**（§9）。整轮若仍被平台终止（顶层 catch 不执行），**轮首心跳**会先落下 `cleanup:lastRunAt`、而**完成戳** `cleanup:lastCompletedAt` 只在轮尾写 ⇒ 「只开始了、没跑完」在 `/ui/api/info` 上显示为**「清理未完成」**（2026-09-26 起界面按完成戳判定；F1 后首批另有 5 条探路，见 §7.1）而不再静默（原始发现见 `docs/free-plan-audit.md` §5 P0-3，其建议 1/2 已落地；"可见"≠"不超"—— 单条 1 MiB 内联文本仍挡不住 10 ms）。积压大时升级 Workers Paid，或放宽 `wrangler.toml` 的 Cron 间隔；对齐上游的清理语义不变，只是收敛速度受平台约束。⚠️ 但**行字节预算两档都生效、且是同一个常数**（运行期读不到账号计划 ⇒ 不可能分档）⇒ 升级 Paid 只解掉 CPU 这一档的平均预算、**不会放宽它**，大记录库的收敛在 Paid 上同样慢（见 §9 与 D40）。**实测回填（2026-09-25）**：本账号 Cron 264/264 全部 success、单轮 CPU 均值 7.46 ms / 峰值 19.5 ms（**未被终止**）⇒ 当前是「预算到位、但尚未成为约束」的状态；数据见 `docs/free-plan-account-facts.md` §3.4 |

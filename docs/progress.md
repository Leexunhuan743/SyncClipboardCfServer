...
下新增 **`### 已知限制（会影响使用的行为差异）`** 一张表（8 行），作为这批差异的**唯一用户面清单**，
每行写「现象 / 与官方的差别 / 影响与对策」：默认不按时间过期（保留期 0，收藏/置顶不参与裁剪）、
删除后仍占存储（软删保留数据 ≤30 天）、单请求体 48 MiB 封顶、Group 与内联文本的额外封顶
（含本轮新增的**单条目 24 MiB**）、升级前记录不回带完整性哈希、打错地址的 401 计入失败限速、
不能导入官方 `history.db`、畸形压缩包口径不同。技术依据仍以 `docs/protocol.md` §10 为唯一登记处
（表头已写明指向）。

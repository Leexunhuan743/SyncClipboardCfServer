...
|---|---|---|---|
| 业务代码 | `src/**/*.ts` | 6471 | 27 |
| 测试代码 | `test/**/*.{ts,mjs}`（含 `support/` 与一次性脚本） | 7724 | 22 |
| 文档 | `README.md` + `docs/*.md` | 4348 | 8 |
| **合计** | | **18543** | **57** |
...
- **§1.3 批量操作**：`batch-delete` 泛化为 **`batch-update`**（`{items, update:{starred?|pinned?|isDelete?}}`，
  ≤200 条，逐条走 `applyHistoryUpdate`）。界面选择条按视图给出不同动作：活跃视图「收藏 / 置顶 / 删除选中」
...
跨标签页收敛不依赖 Hub。
（协议侧的 `clear` 顺手做了一处**效率**修复：逐条 `deleteHistoryWorkingDir` 换成一次
`clearHistoryData()`（前缀列举 + 批量删），同规模子请求从约 2000 次降到个位数——原来的写法在本机
1009 活跃记录的规模上会**中途失败**。）
...
- **已接受的取舍（正式记录）**：`clearAllHistory`（协议 `DELETE /api/history/clear` 与 UI `scope=all` 共用）
  先删行、再清整棵 `history/` 前缀，两次调用之间的毫秒级窗口里若有并发上传落地，**那一行的 R2 对象会被前缀清掉**
  （行还在 ⇒ 表现为 `hasData` 但对象缺失，正是 `/ui/api/integrity` 能查出的形态）。逐条删目录的旧写法没有这个窗口，
  但它在 1000 条规模上会**超单次调用子请求上限而半途失败**——两害相权取前缀清理，残留由完整性自检与孤儿阶段兜底。

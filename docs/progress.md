...
## 159. 未定位的偶发：全量套件在 2/17 轮里各红 1 条（2026-09-22，待下次抓到）

**现象**：同一天里有 **2 轮**全量套件红 1 条，两次都是 `Test Files 1 failed | 21 passed`、
`Tests 1 failed | 451 passed (452)`；**两次都没抓到测试名**（当时的命令只打印了 `tail`，
...
```
**结论**：登记为**已知偶发**。它会让 ~12% 的推送在 CI 上红一次（部署被跳过、重推即可恢复），
不是产品缺陷；下次出现时按上面那条命令取名字，再决定是加超时余量还是修判据。

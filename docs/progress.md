...
下新增 **`### 已知限制（会影响使用的行为差异）`** 一张表（9 行），作为这批差异的**唯一用户面清单**。
列按用户要求**两侧逐列对写**：`方面 | 官方服务端 | 本项目（cfserver） | 影响与对策` —— 覆盖历史过期、
删除语义、单请求体上限、文件夹/内联文本封顶、升级前记录的完整性哈希、未知路径鉴权、`history.db` 迁移、
畸形压缩包口径、错误响应体。技术依据仍以 `docs/protocol.md` §10 为唯一登记处（表头已写明指向，
每一行的上游依据在 §10 里都带 `文件:行`）。

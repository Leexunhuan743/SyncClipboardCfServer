...
- 97. `public/` 逐 hunk 核实台账：208 处全部过了一遍（2026-09-20，同上一句指令，对象换成 `public/`）
- 98. `public/` 台账的**独立复核**：重读 4708 行 diff，新查 3 条（2026-09-20，用户「逐个核实…直到确保真的全部都完成。禁止跑测试。禁止脚本」）
- 99. `test/` 逐 hunk 核实台账：79 处全部过了一遍，查出的 4 条**全在注释里**（2026-09-20，同上一句指令，对象换成 `test/`）
- 100. `docs/` 逐 hunk 核实台账：101 处全过，查出的 15 处**全在"指代/引用"**上（2026-09-20，同上一句指令，对象换成 `docs/`）
- 101. `src/` 台账的**独立复核**：范围重算逐位吻合，但台账对自己"修复后状态"的描述错了 4 处（2026-09-20，同上一句指令，对象换成 `src/`）
...
| 2 | `src/ui/query.ts:457`（今 :473） | `下面那句\"大小写不敏感\"的过滤…` | 批改脚本的转义残留：`\"` 是**文件里真实存在的两个字符**（不是 Markdown 观感）⇒ 应改成「大小写不敏感」。⚠️ **订正（2026-09-20，§96）**：当时**只写了结论、没有真的落地** —— 全仓没有任何 spec 改过这一行，文件里至今仍是 `\"`；判断本身没错（确实只有一处），错在把「该改的」写成了「已改的」。§96 已改，并把「自述已改」当判据复核了一遍 |

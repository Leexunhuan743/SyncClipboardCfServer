...
34. **顶栏在向下滚时整条滑出，向上滚立刻回来**（2026-09-22 用户定形：「滚动的时候最上面这一个
    ...
    · **有选中时不折**：那一刻头栏里的批量条正开着（正要点「移动到回收站」）。这一条**纯 CSS** 表达
      —— `html[data-header="hidden"]:not(:has(.results__selection-count))`（判据与"选中态"同源），
      于是"选中 ⇒ 顶栏回来"**不需要任何 JS 钩子**，也不会漏掉清空选择集的那几条路径。
      ⚠️ 于是存在一处**有意的不对称**：选中时 `data-header` 仍是 `hidden`、而视觉上已展开
      （探针把这条不对称本身也钉住了）。
    ...
    · 判据：探针的 `HEADERFOLD`（滑出 / 不留空带 / 向上滚展开 / 选中时不折）+ `stickyBg`（不透明底）。
...
| 顶栏折叠（`header-fold`） | **滚动状态变化**：向下滚且离开顶部 120px ⇒ 整条滑出；向上滚 / 回到顶部 ⇒ 滑回（**有选中时不折**，见 §3.3 第 34 条） | 位移走 `transform`（合成层、不动布局）；状态由 `html[data-header]` 表达（**只写属性，不每帧改样式**）；`prefers-reduced-motion` 下瞬时到位；省下的 56px 由 `--header-h-effective` 让给头栏与表头 |

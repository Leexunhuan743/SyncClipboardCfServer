...
| `js/components/toolbar.js` | 类型分段筛选（每枚 chip 带图标 + 计数，窄屏折叠文字见 §3.3 #23）、收藏筛选、搜索（含清空按钮、`Esc` 清空、`focusSearch()` 供快捷键调用、`/` 键帽提示）、每页条数、刷新（按钮自带进行中态）、**快捷键帮助的可见入口**（`aria-label="键盘快捷键"` 的「?」按钮，2026-09-23 补，见 §3.3 #36） |
| `js/components/list.js` | 结果区：表格、行、**四个固定槽位的行内动作**（预览/复制/下载/移动到回收站，缺失的动作放等宽占位——同一动作在每行的位置恒定；文本行的第 3 槽分两支：有原文件是「下载」（原字节+原名）、内联文本是「下载文本」（正文存成 `.txt`））、排序表头、选择条（Shift 范围选择；批量按钮带**进行中态**）、**一键复位筛选**、**回收站视图操作带里常驻的「清空回收站」**（没有选中时那条带子只装它；位置在「取消选择」左侧，见 §3.3 #31）、空状态；**同一视图内的刷新按行对账**（内容未变的行不重建，见 §3.3） |
...
29. **预览框里的「编辑」是两态机 预览 ⇄ 编辑，保存 = 新建一条记录**（2026-09-22，ADR D30，
    ...
    · **保存后不关框**：正文换成刚保存的那段，另**弹出一条提示**「已保存为新记录（N 个字符）。
      原来那条仍在历史里，列表已刷新。」（`.dialog__toast`，复用全局提示条的 `.toast` 外观与
      `motion.css` 的进出动画，2.6s 后自己收掉；文案仍住在 `messages.js` 的 `textSavedNote`），
      列表在框**背后**静默刷新。
      ⚠️ **它是对话框内的提示条，不是全局 `#toasts` 那一条**（用户 2026-09-22 要"像复制最近一条那样
      弹出来的"）：模态 `<dialog>` 在 **top layer**，`#toasts` 在它下面 ⇒ 模态开着时那条提示被盖住。
      三条路都实测过：① 直接往 `#toasts` 塞 —— `elementFromPoint` 拿到的是 dialog / 它的 backdrop
      （ADR D30 早记过）；② 把 `#toasts` 改成 `popover="manual"` —— 已显示的 popover 再
      `showPopover()` 是 no-op、不升层；③ `hidePopover(); showPopover()` 也不行（后开的模态仍在上面）。
      所以它挂在对话框自己的槽位（正文区与页脚之间，`flex: none` 恒在视野内），
      `role="status"` 由它自己播报（宿主不是 `#toasts`，不适用"宿主已宣布就别再嵌实时区域"那条）。

...
| 5k | **回收站**（查看已删除 / 恢复） | **本轮新增**：`deleted=true` 只看已删除行（`src/ui/query.ts`）；软删**保留数据文件**（ADR D29），恢复把记录连同数据一起拿回来，见 §5 第 4 条 |
...
开启态资源未命中必须回落出 404 页；**每个界面挂载点都在 `run_worker_first` 里**（2026-09-18 补的第三条、2026-09-19 改名后扩到三个、2026-09-21 起是四个——加 `ui_shared`；挂载点从 `public/` 动态发现 ——
此前文档写着"删掉它会测试即红"，而实际上没有任何测试在读这份配置）。
线上每次部署后由 CI 冒烟按开关断言四个挂载点的页面与入口 JS：
`/ui/`（跳转壳）、`/ui_v1/`、`/ui_v1/js/main.js`、`/ui_v2/app/`、`/ui_v2/js/boot.js`、`/ui_shared/brand/favicon.svg`。
```
...
`public/` 下共 86 个资源，分五部分（2026-09-21 新增共用层后）：

| 部分 | 文件数 | 说明 |
|---|---|---|
| **V1**（`public/ui_v1/`，**默认界面**） | 35 | 默认入口，挂载 `/ui_v1/`；2026-09-17 修复接口前缀、重做密度与移动端，2026-09-18 接手默认跳转、并把用户文案收回本地 `js/messages.js`（见该目录 `README.md`）；2026-09-21 新增 `js/components/tooltip.js`（hover 预览浮层，见 §3.3 硬约束 #26） |
...
| **共用层**（`public/ui_shared/`，2026-09-21 新增） | 4 | 两版**唯一**的共享面：`brand/favicon.svg`、`brand/favicon-32.png`、`brand/apple-touch-icon.png`（两版此前逐字节各存一份）与 `js/icons.js`（两版图标表的**并集**，2026-09-23 复核为 33 键：两版都用 **24** + 只 V1 用 4（`edit` `arrowUp` `image` `push`）+ 只 V2 用 5（`chevronDown` `dots` `filter` `list` `settings`）；两版都不用的键按死数据删除）。**2026-09-23 的一处变动**：`inbox` 从"只 V2 用"变成"两版都用"（V1 的类型 chip「全部」借它当图标 —— 见 §3.3 #23 为什么不用 `list`/`clipboard`）。挂 `/ui_shared/`，与其它三个挂载点受**同一个** `UI_ENABLED` 管；允许放什么见 §3.4 |
...
| `js/components/stats.js` | 统计条（三个真实数字：记录 / 已收藏 / 存储占用，**数字在上、标签在下**）；明细行 = 「全库 N 条」+ **排障面**（最近同步 / 时钟差 / 清理状态 —— 清理状态自 **2026-09-26** 起按**完成戳**判「清理正常 / 清理未完成（多为被平台终止）」，`progress.md` §57.2、§187）+ 近 14 天活动趋势（回收站入口已按用户要求删除，见 §54.3） |
| `js/components/toolbar.js` | 类型分段筛选（每枚 chip 带图标 + 计数，窄屏保留文字、放不下时换行，见 §3.3 #23）、收藏筛选、搜索（含清空按钮、`Esc` 清空、`focusSearch()` 供快捷键调用、`/` 键帽提示）、每页条数、刷新（按钮自带进行中态）、**快捷键帮助的可见入口**（`aria-label="键盘快捷键"` 的「?」按钮，2026-09-23 补，见 §3.3 #36） |
| `js/components/list.js` | 结果区：表格、行、**四个固定槽位的行内动作**（预览/复制/下载/移动到回收站，缺失的动作放等宽占位——同一动作在每行的位置恒定；文本行的第 3 槽分两支：有原文件是「下载」（原字节+原名）、内联文本是「下载文本」（正文存成 `.txt`））、排序表头、选择条（Shift 范围选择以记录 key 作锚、操作当下按现有顺序定位；批量按钮带**进行中态**）、**一键复位筛选**、**回收站视图操作带里常驻的「清空回收站」**（没有选中时那条带子只装它；位置在「取消选择」左侧，见 §3.3 #31）、空状态；**同一视图内的刷新按行对账**（内容未变的行不重建，见 §3.3） |
...
| `js/components/preview.js` | 预览对话框（文本全文 / 图片原图 / 不可用态）；点背景关闭、**打开与退出编辑后焦点落在正文框**（`.dialog__body` 带 `tabindex="-1"`，滚轮与键盘都能滚；滚轮落在页眉/页脚上时也会转给正文，2026-09-22）、长文本先给加载态；主操作文案与行内统一（「复制文本」「下载文本」「复制图片」「下载」）。**2026-09-22（ADR D30）起文本多一个「编辑」态**：两态机 预览 ⇄ 编辑（等宽 `<textarea>`、页脚换「取消 / 保存」、保存 = 新建一条记录、保存成功后**改指向新记录**、Esc 只退编辑不关框、保存途中不许关框，见 §3.3 #29）；头部（类型徽标 / 字符数 / 时间）由 `renderHead()` 单点绘制。**页脚最左是视图级销毁动作**（活跃记录「移动到回收站」/ 回收站记录「彻底删除」，钉在左缘、与行内槽 4 同一个 action，见 §3.3 #32） |
...
| `js/components/shortcuts.js` | **快捷键帮助浮层**（工具栏那枚「?」按钮或 `?` 键打开）+ 目录渲染：**四组**（列表页 / 行内 / 预览框 / 编辑正文）逐条列键帽与说明。目录与派发器读**同一份表**（`main.js` 的 `listShortcuts()`）⇒ "帮助里写的"与"实际绑的"不会漂移（取向见 §3.3 第 36 条、ADR D37） |
...
| `js/main.js` | 装配点：唯一知道「谁是谁」的地方；`actions` 返回「是否做成」供组件呈现，另承载**整张快捷键表**（`listShortcuts()`，含 `/ ? r t n p Esc`）、翻页/改筛选后的滚动定位、**首屏合成快照**（`refreshOverview`）、批量复制、顶栏「复制最近一条」、失败路径的「重试」接线（§57.2 / §58） |
| `js/login.js` | 登录页逻辑 |
...
6. **列表 / 统计 / 概览 / 活动趋势 / 变更信号 / 预览各有 `latest.js` 的守卫**：`begin()` 拿 ticket、`isCurrent()` 通过才允许落地，且 `catch` 里先看 `signal.aborted`（被取代不是失败）。少了这一步就是「陈旧响应覆盖新状态」——实测复现过：URL 与筛选控件说 Text、列表里是 46 行图片，且不会自愈。

7. **推送是轮询的加速器，不是替代品。** 可见时用 `/ui/api/hub-ticket` 换票据、建 WebSocket
   （`js/signalr.js`：`\x1e` 分帧、30 秒心跳——DO 静默 60 秒即断、退避重连），收到广播后去抖
   300ms，再刷新列表、统计与活动趋势；同一份列表按行对账逻辑也用于轮询。批量写的广播已合并
   为每批一次（ADR D33）；去抖仍负责吸收连续批次与其它设备同时发来的通知。连接在线时轮询降为
   60 秒看门狗；断线、拿不到票据（503）、环境不支持——失败都自动回到 10 秒轮询，且**连续失败
   5 次后进入 10 分钟冷却**（坏环境里每 ≤60 秒白试一次 ≈1.4k 请求/天），冷却结束或回前台时重试。
...
10. **深链接 `#Text-<hash>`**：`hash` 空闲（筛选状态在 query string），打开即预览那一条；
    写入片段前对 hash 编码，读取时解码并拒绝路径分隔符（服务端允许非十六进制 hash）；
    连续打开链接、改成无效片段与请求途中关框都以最新一次操作为准，迟到响应不能重新开框。关闭时清掉片段，
    避免刷新又弹出上一条。
    ⚠️ 清 hash 依赖 `dialog` 的 `close` 事件：headless 的隐藏标签页里该事件不派发（页面被冻结），
    故这条行为**只在真实浏览器可靠**，本机浏览器回归未覆盖到它。

11. **列表更新一帧落地，不做视图过渡、不播入场动画。** 视图过渡要对整个结果区做布局/样式快照
    （实测 6× 降速下一次「什么都没变」的切换多花 ~60ms，且随页大小上升），已在 2026-09-18 **移除**。
    ⚠️ **行入场错峰也于 2026-09-22 按用户要求整条移除**（原形态：首屏前 12 行按 `--row-index × 40ms`
    依次从下方 6px 淡入，尾巴 (12 − 1) × 40ms = 440ms）—— 刷新后逐行从上往下冒出来读起来是"慢"，
    不是"来了"；它对应的"内容事件"本来也只是首屏仪式。CSS 的 `row-in` / `data-enter` / `--row-index`
    与 `list.js` 的 `ENTER_STAGGER_LIMIT` 已一并清掉 ⇒ **首屏与后续更新现在都不播入场动画**。
    取舍与判据见 `progress.md` §155。
12. **筛选控件立刻画，列表等响应。** `store` 不触发绘制，而 `render()` 原先只在 `refresh()` 落地后跑 ——
    从点击到响应这段时间里被点的那一段毫无变化（真实网络上是几百毫秒的「点下去没反应」，响应一到整块换掉，
    读起来就是卡顿）。现在 `applyFilters` 在发请求前先 `render()`：控件即时按下、结果区 `data-busy` 变淡，
    行仍是旧的，等响应回来再按行对账。
13. **切换类型不是「新视图」**：`全部 ↔ 文本` 在多数库里是同一批行（线上 74/74），原先每次切换都
    整表重建 + 12 行错峰入场 + 结果区交叉淡入，全是白做。现在只有**首屏（第一份非空结果）**走一次
    整表重建，之后一律按 key 对账（`reconcile`）、不播动画；`viewToken` 那套「视图标记」
    已随之删除——它当年只为入场动画服务，判断「内容是否变」由行签名（`signature()`）负责。
    （那条入场动画本身也已在 2026-09-22 移除，见第 11 条。）
...
17. **行内操作那一栏（预览 / 复制 / 下载 / 删除）的三条硬约束**（2026-09-18）：
    ① **槽位固定**（预览=1、复制=2、下载=3、移动到回收站=4，缺失的用等宽占位；回收站的行是
    恢复=1、预览=2、占位=3、彻底删除=4，见 #27）——
...
23. **窄屏工具栏的两条**（2026-09-18 用户两句）：「每页条数 + 刷新」是**一组**（同一个
    ...
    **窄屏那一排 chip：定形为"不折字"**（2026-09-23，用户四句话："窄页面很多东西都会折叠文字部分，
    你看看他们的的实现，应用到全部" → "全部|文本|图片|文件|组合 不需要折叠文字" → "保留图标" →
    "为什么收藏和回收站 收起文字之后间隔变得那么的大"）：
    · **七枚 chip 的文字全都留着**。类型五枚是 图标（`inbox`/`text`/`image`/`file`/`group`）+ 文字 +
      计数；「收藏」「回收站」是 图标 + 文字。**图标与文字是叠加关系、不是替代关系** ——
      图标是扫读加速器（用户明确要求保留）。
    · **折叠为什么不成立**（用户最后一句问的就是这个）：那一行是
      `[左对齐的视图组][spacer flex:1][右对齐的分页组]`（分页组 `margin-left: auto` 贴行尾是
      2026-09-18 定下的、探针 `gapToRight: 0` 在钉）。收起两枚视图 chip 的文字会腾出 ~115px，
      而这 115px **全部**被 spacer 吸收 ⇒ 行中间出现一个大洞，看起来比不折更糟。
      要折就得同时改那一行的对齐契约，而那件事本身没有收益（这一行放得下）。
    · **真正需要修的是"被裁掉"**：实测 390×844 —— 五枚带图标的 chip 合计约 470px > 可用 343px，
      而这一组**没有**自己的滚动条（`.segmented` 是 `inline-flex` + 默认 `nowrap`）⇒ 溢出部分被
      `html { overflow-x: clip }` **直接裁掉**（用户截图里"最后一个「组合」被切一半、还滚不到"）。
      修法：`.toolbar .segmented` 在 ≤560px 允许 `flex-wrap: wrap`（390 档折成两行、工具栏 168 → 200px，
      477 档仍是一行 168px），内边距 12→8 让每行多塞得下一枚。
    · **「全部」的图标不用 `list`**：`list` 与 `text` 在 14px 下几乎同形（都是横线，截图里分不出来）；
      `clipboard` 是两版的品牌标记（V1 顶栏、V2 appbar 都在用），也不能借 ⇒ 用 `inbox`
      （托盘，与 Gmail 的"收件箱 = 全部邮件"同一取向）。
    · 名字与数字一并写进 `aria-label`（类型 chip 形如"文本 631"，计数未知时不写数字）。
    判据：探针的 `PAGERBAR` 一行（七枚 chip 的文字都**在** / 计数在 / `aria-label` 齐全 /
    类型那排不溢出工具栏右缘）。
...
27. **回收站的行内出口各占一个固定槽位，且都过确认框**（2026-09-21 定案，`progress.md` §125）：
    · 行内动作三个 —— **恢复固定槽 1**（本视图的主操作）、**预览固定槽 2**（2026-09-22 用户要求
      「放在删除和撤回之间」：它紧挨「恢复」的右边，与槽 4 那个不可撤销的之间留一整格 ——
      常用的无害动作不该贴着危险动作，与 #17② 的触屏间距同一条判据）、**彻底删除固定槽 4**
      （与活跃视图的"移动到回收站"同位），槽 3 是等宽占位。理由与活跃视图那四个固定槽同一条：
      动作按类型追加会让"哪个按钮在哪一列"逐行不同，而切换视图后误按的代价是
      "把记录恢复出去"或"把它永久删掉"。**预览不占槽 1**：槽 1 在这个视图里已经是「恢复」，
      挤走它等于把"进回收站第一件事"换位（代价大于"预览的列位跨视图不一致"）。
    · 两处入口：行内「彻底删除」+ 选择条「彻底删除选中」（回收站视图专有，与「恢复选中」并列；
      「清空回收站」2026-09-22 起常驻在操作带里、「取消选择」左侧，见 #33）。**没有"彻底删除
      少量/中量"的入口时，用户只能整罐倒** —— 这正是它存在的理由（2026-09-21 实测定案）。
...
28. **长批量在途可中止，中止点是"批"的边界**（2026-09-21，`progress.md` §127）：只有传入
    `cancellable: true` 的分片批量在确认框**在途**时把「取消」变成「**中止**」；单条与清空操作
    不接中止信号，在途时禁用取消（✕ 与 Esc 同样挡住 —— F2 的理由不变：在途关框会让调用方把"已成功"
    ...
    · 写批量只在选中数 **>100**、确实会发下一片时开放「中止」（收藏/置顶/删除/恢复/彻底删除）；
      批量复制是读请求，可连在途那一片一起取消。单条动作与清空操作是单次请求，
      发出后无法保证服务端停止，因此不显示会误导用户的「中止」。
    · **没有对话框的那几个，中止键在选择条上**（2026-09-22，ADR D34）：那枚按钮在途时变成「中止」
      ...
      背景列表刷新时保留在途按钮并暂时禁用其它批量按钮，结算后才重建选择条；无对话框批量任务互斥，避免共用的
      中止钩子指到另一项操作。
...
29. **预览框里的「编辑」是两态机 预览 ⇄ 编辑，保存 = 新建一条记录**（2026-09-22，ADR D30，
    ...
    · **保存后不关框**：正文换成刚保存的那段，另弹出全局提示「已保存为新记录」
      （短文本附准确字符数）；列表在框**背后**静默刷新。`#toasts` 宿主会临时
      停靠进最上层对话框，确保模态打开时提示仍可见、可操作（ADR D36；此前的 `.dialog__toast`
      已删除，见 `progress.md` §167）；文案仍住在 `messages.js` 的 `textSavedNote`。
    ...
    · **保存途中不许关框**（2026-09-22 自审补，与 `confirm.js` 的 F2 同源）：`saving` 期间 ✕、
      「点背景」、取消键与 Esc 都被挡；Esc 不能先退出编辑、再靠第二次 Esc 关框。请求失败时
      原因要显示在编辑框旁，若在途关框就会把错误与未保存的输入一起丢在屏幕外。
    ...
    · **头部、复制提示与保存提示使用同一字符口径**（ADR D39，`progress.md` §175）：服务端
      `size` 是 UTF-16 码元数，不能标成用户看到的「字符」。正文不超过 20,000 码元时用
      `charCount()` 按字素簇计数；大文本在头部写「长文本」、提示条省略数字，避免为一句说明
      全量运行 `Intl.Segmenter`（1.1 MB 正文曾实测需 169ms）。深链接加载壳缺少 `size` 时
      头部暂不写数字；从列表打开被截断的长文本时，加载壳同样不拿那 500 字冒充全文。
      全文到达后才按真实正文显示。
...
31. **「删除」只指不可撤销的那一档；软删一律叫「移动到回收站」**（2026-09-22 用户定案）：
    · **两个动词各有唯一含义** —— 移动到回收站 = 软删（进回收站、30 天内连同数据文件可恢复）；
      彻底删除 / 清空回收站 / 清空全部历史 = 不可撤销（D1 行 + 数据目录立刻没了）。同屏出现
      "删除"却指的是可恢复的那件事，用户没法从按钮上判断按下去还能不能后悔 —— 这正是这条要消掉的。
    · 落到名字上：行内槽 4 的 `aria-label`/`title`、选择条那枚批量按钮、`preview.js` 页脚最左那枚、
      两个确认框（标题 / 正文 / 确认键）**逐字一致**都是「移动到回收站」；提示条说「已移动到回收站 N 条」；
      回收站视图的空态与工具栏 chip 的 `title` 说「移动到这里的记录…」；部署信息里的计数行写「回收站 N 条」。
      不可撤销那一档继续用「彻底删除」（`purgeConfirmSpec` / `batchPurgeConfirmSpec` / `clearHistorySpec`）。
    · **文案单点**：这些句子住在 `messages.js`（`deleteConfirmSpec` / `batchDeleteConfirmSpec`），
      界面上的按钮名与它同源；两版逐字一致由 §3.4 的对等守卫钉住。
    · `confirm.js` 的 `confirmLabel` **缺省值改成「确认」**（此前是「删除」）：动作名必须由调用方给，
      写死一个动词会让"忘了传"的那一处显示成一个错的、且强度更高（不可撤销）的动词。
    · 判据：`test/ui-logic.test.ts` 的「文案对齐服务端语义」两条（软删的确认键必须含「回收站」且不含
      「彻底」，硬删必须含「不可撤销」且不含「30 天内」）。
32. **预览页脚的最左一格是"视图级销毁动作"，钉在左缘**（2026-09-22 用户两次定形）：
    · 位置：`.dialog__foot` 里**排在 spacer 之前**（`.dialog__foot-spacer` 是 `flex: 1 1 auto`，
      排在它后面的才被推到右边）⇒ 这一枚贴左缘、其余动作（编辑 / 复制 / 下载）仍靠右。
    · **≤560px 时预览操作只显示图标**（2026-09-23 窄屏实测修复）：四枚操作各有
      `data-action` 与 `aria-label`，隐藏 `.btn__label` 后仍可被读屏识别；成功态由
      `flashSuccess()` 临时更新可访问名再还原。编辑态的「取消 / 保存」没有 `data-action`，
      仍显示文字。390px / 320px 下四枚图标都必须留在对话框内。
    · 两档同一格：**活跃记录 =「移动到回收站」**，**回收站里的记录 =「彻底删除」** —— 与行内槽 4
      同位置、同 action（`main.js` 的 `deleteFromPreview` / `purgeFromPreview` 包一层"做成后关框"，
      失败不关框）。切换视图时"最左那枚"换的是动词而不是位置。
    · **初始焦点落在正文框，不落在这一枚上**（2026-09-22 用户要求）：`.dialog__body` 带
      `tabindex="-1"`，打开预览与退出编辑后 `body.focus({ preventScroll: true })` ——
      滚轮与键盘（↑↓ / PageUp·Down / Space）立刻能滚正文（实测 1.1 MB 记录：`scrollHeight` 205937 /
      `clientHeight` 576，PageDown 后 `scrollTop` 504）。`-1` 不进 Tab 顺序，Tab 仍先到页脚那几个按钮；
      编辑态里焦点落在 `<textarea>`（原有行为）。**加载态**（长文本要先取全文）也落在正文框 ——
      此前落在 ✕ 上、等全文到了再跳过来，用户看到的是"焦点过一会儿才到位"。
      **不是**"落在第一个按钮上" —— 页脚最左那枚是销毁性的，
      而按钮顺序还会随记录类型变。每次打开把 `body.scrollTop` 归零（它是常驻节点，留着上一条的位置）。
    · **滚轮落在页眉/页脚上时转给正文**（2026-09-22 用户实测报的"点完编辑后滚不动"）：`dialog` 上挂
      `wheel`（`passive: false`），**指针不在 `.dialog__body` 内**才接管 —— 把 `deltaY`（按 `deltaMode`
      换算成像素：1=行→×16、2=页→×容器高）加到"当前滚动容器"（编辑态是 `<textarea>`，其余是正文框）上并
      `preventDefault()`；容器没有可滚内容时**不拦**（不留一次被吞掉的滚轮）。根因是可测的：点完按钮后指针
      仍停在页脚，而页脚不可滚、模态又压着背后页面 ⇒ 浏览器去滚了**背后的列表**（实测 `pageScrollY` 从
      369 变 1287）。修后实测：预览态 over 页脚 `bodyScrollTop 0 → 320`（页面不动）；编辑态 over 页脚
      向上滚 `taScrollTop 1539 → 1059 → 579`。
    · 判据：`preview.js` 的 `renderViewActions`（`destructive` 分支 + 末尾的 `body.focus`）与 `scrollTarget()`。
      探针的 `PRVCLOSE` 行只断言"关闭后正文与页脚都归零"，故页脚子节点数由 4（spacer +
      编辑/复制文本/下载文本）变 5 不影响它。

33. **回收站视图的「清空回收站」常驻在操作带里、「取消选择」的左侧**（2026-09-22 用户三次定形）：
    · 位置：它住在结果区头栏那条操作带（`.results__selection`）里、**紧挨「取消选择」的左边**；
      没有选中时那条带子**照样显示**（只装这一枚，没有「已选 N 条」、没有「取消选择」）。
      两种状态的位置恒定：`[已选 N 条][恢复选中][彻底删除选中][清空回收站][取消选择]` ↔ `[清空回收站]`。
    · 为什么不是"选择条里的一枚批量按钮"（旧形态）：那条带子只在勾中至少一行时渲染 ⇒ "想清空整罐
      得先勾一条"，而且读起来像"对选中项动手" —— 它实际清的是**整个回收站**（`api.clear('trash')`）。
    · 可见性判据是"回收站里到底有没有东西"，两条证据取并集：`deletedCount`（统计里的**全库**已删计数，
      不受当前筛选影响 ⇒ 回收站里套一个 0 命中的类型筛选时仍给真值）与 `total`（兜住统计未落地的窗口）。
      两者都为零才隐藏（空回收站那屏已有「回收站是空的」+ 说明 + 返回入口）。失败态（`showError`）一并收起。
    · **头栏的"选中态底色"判据跟着改**：`layout.css` 用 `.results__head:has(.results__selection-count)`
      （「已选 N 条」在不在），不再用 `:has(.results__selection:not([hidden]))` —— 后者会让回收站视图的
      头栏在没有选中时常驻着"选中态"的底色。
    · 窄屏（≤560px）它走**已有的那条**规则（`.results__selection .btn:has(svg) .btn__label`）收成图标：
      它有 trash 图标与 `aria-label`（「清空回收站」），与批量按钮同款；不收的话
      「回收站 · 共 N 条 / 清除筛选 / 清空回收站」三件在 360px 下合计约 369px，会压 `.results__count`
      折行、把 45px 的头栏顶高。
    · **头栏恒吸顶**（2026-09-22 用户第二次定形）：`.results__head { position: sticky; top: var(--header-h); z-index: 9 }`
      —— 未选中时显示「共 N 条记录（+ 清除筛选）」，选中时**同一行**就地换成「已选 N 条 + 批量按钮 +
      取消选择」，两态同高同位置 ⇒ 选中前后**零位移**。表头的吸顶偏移因此是**常量**
      （`.table th { top: calc(var(--header-h) + var(--results-head-h)) }`），卡片档（≤860px）表头
      仍是 `top: auto`（随页面滚走的紧凑工具条，不存在重叠）。
      ⚠️ 更早一版是"**选中才**吸顶"：那一刻表头要跟着让一格 ⇒ 勾一行会把表头连同正在读的内容
      整体推下 45px（2026-09-22 实测：滚到 800 时表头 top 56 → 101），用户否掉了它。
      代价如实记：那一行**常占 45px**（上一版正是为省这 45px 才做成"选中才吸顶"）。
      底色：基础规则常驻 `background: var(--bg)`（**吸顶的前提** —— 没有不透明底时下面的行会从
      它背后透出来，2026-09-22 用户截图报过这条；判据见探针的 `stickyBg`），选中态由
      `:has(.results__selection-count)` 换成 `--accent-soft`（特异性更高）：**位置常驻，语义不常驻**。
      实测（1440×900，滚到 y=800）：未选中 `headY=56`、表头 101；选中后 **`headY=56`、表头 101 都不变**
      （只有 band 在同一条带子里出现），五枚批量键都在 y=60 ⇒ **零位移**。改前那一版是
      未选中 `headY=-567`（滚走）、选中才 56，而表头被推到 101 —— 那 45px 的跳变正是被否掉的原因。
    · 判据：`list.js` 的 `headEmptyTrash` + `renderHead` 的 `showResident`/`headSelection.hidden` +
      `layout.css` 的 `.results__head`（恒吸顶）与其 `:has(.results__selection-count)`（底色）+
      `tokens.css` 的 `--results-head-h`；
      实测四态头高恒 45px（见 `progress.md` §150）。

34. **顶栏在向下滚时整条滑出，向上滚立刻回来**（2026-09-22 用户定形：「滚动的时候最上面这一个
    折叠起来」）：
    · **触发**：向下滚**且离开顶部 120px** 才折；**一向上滚立刻展开**；离顶部 8px 内一律展开
      （第二道保险，免得亚像素滚动与惯性回弹时开合抖动）。方向判据带 **2px 死区**。
    · **形态**：整条滑出（`transform: translateY(-100%)`，合成层位移、不动布局），省下的 56px 让给
      下面两条 —— 吸顶链上三处（`.results__head` 的 `top`、`.table th` 的 `top: calc(…)`、
      `.results` 的 `scroll-margin-top`）一律走 `--header-h-effective`（折叠时归 0）⇒ **不留空带**
      （实测：折叠后头栏贴 0、表头贴 45、顶栏 −56）。
    · **多选时不弹回来**（2026-09-22 用户第二次定形：「这一行多选的时候不需要弹出来」）：
      折叠态只由**滚动**决定，与选择集无关。早先那版加了"选中就展开"的护栏（理由是"正要点
      「移动到回收站」"），实测发现它**自相矛盾** —— 带选中滚动时勾一行，整条吸顶链会从
      `0/45` 弹回 `56/101`，把内容**推下 56px**，违反第 33 条的「选中前后零位移」。
      而批量按钮在**结果区头栏**里、不在顶栏 ⇒ 折叠顶栏不影响操作。那条
      `:not(:has(.results__selection-count))` 守卫已删除（探针据此判"勾一行零位移"）。
    · **状态 / 过渡 / 触发分家**：状态在 `layout.css`（两条规则）、过渡在 `motion.css`
      （`prefers-reduced-motion` 下瞬时到位）、触发在 `main.js` 的 `syncHeaderCollapse()`
      —— **只在状态变化时写一个属性**，不每帧改样式；监听器 `passive`。
    · 代价如实记：滚动时顶栏上的**实时推送状态**与**部署信息入口**看不见 —— 想看就往上滚一点
      （或滚回顶部）。「失去联系」另有横幅（轮询失败时显示在结果区上方），故滚动中不会丢掉连通性提示。
    · 收益与取舍：滚动时压屏的 chrome 桌面 **148 → 92px**、窄屏 **101 → 45px**。
      **被否**：压成细条（56 → 40，只省 16px）、纯淡出不改高度（省 0）。
    · 判据：探针的 `HEADERFOLD`（滑出 / 不留空带 / 向上滚展开 / **多选时仍折叠、零位移**）+
      `stickyBg`（不透明底）。

> 前台另有两条与本轮无关但同样承重的旧约定：正文一律走 `textContent`（`dom.js` 不提供插入 HTML 的途径，见 §7）；行入场动画**已于 2026-09-22 移除**（见上面第 11 条），故"轮询刷新不重放入场"这一类顾虑不复存在。

35. **对话框开着时的提示条：把 `#toasts` 宿主搬进最上层对话框，而不是在框里另造一条**（2026-09-22，
    用户："这个根本不是真实的toast"；ADR D36）：
    · **为什么必须搬**：模态 `<dialog>` 在 **top layer**。宿主留在 `body` 下时，提示条既被半透明
      `::backdrop` 压暗、又**完全收不到点击**（实测：点提示条中心，命中的是 `dialog`）。
    · **被否的两条**：① 在对话框里另造一条 `.dialog__toast`（2026-09-22 之前那版）—— 位置与
      "真提示条"对不上，且是同一件事的第二份实现；② `popover="manual"` —— 模态把 top layer
      **之外**的 popover 也置为 inert，实测点不中（`hits=[]`）。
    · **落点**：宿主挂进 `.dialog__foot`，CSS `position: absolute; bottom: 100%` ⇒ 浮在页脚**之上**、
      水平居中，不参与对话框的列布局、也不盖住页脚按钮。
    · **搬的时机与"谁在最上层"**（2026-09-23 修，两处都是实测出来的洞）：
      ① 停靠挂在 `<dialog>` 的 `toggle` 事件上（**捕获阶段** —— `toggle` 不冒泡），而不是只在
      `show()` 里搬。只在 `show()` 里搬时，"提示条**先**显示、对话框**后**打开"这一档的宿主留在
      `body`：用户看得见那条提示、却点不动它，**而且点下去命中 backdrop 会把对话框关掉**
      （实测：真实鼠标点「重试」的位置 ⇒ 预览框被关闭）。现在框一开、一关都会重算落点；
      框关了宿主回 `body`（宿主仍是 `position: fixed`，视觉落点不变，剩下的提示条不会跟着框消失）。
      ② "最上层"按**打开顺序**取（`toggle` 维护的栈），不是按文档序。四个对话框是模块求值时按
      创建顺序（confirm → preview → info → help）append 进 `body` 的，而 `querySelectorAll('dialog[open]')`
      给的是**文档序** ⇒ "预览开着、确认框开在它上面"这一档里，文档序在后的那个反而是**下面**的那个
      （实测：`open[len-1]` 是预览，而该点命中的是确认框），提示条会落进被压住的框里。
    · 副作用（正面的）：`.toast__action`（"重试"那类动作按钮）在对话框开着时**才**真的点得动。
    · 判据：探针 `PRVTOAST`（同宿主 / 临时打开命中区后**真实鼠标**点中它自己 / 浮在页脚之上 /
      2.6s 自收 / 关框后宿主回 `body`）+ `DOCK`（**先提示条后开框**：宿主必须搬进框、
      真实鼠标点「重试」必须命中它自己且**不关掉对话框**；嵌套模态时宿主停在真正最上层那个框）。

36. **快捷键：少而稳，每条都必须有可见的等价按钮**（2026-09-22，用户要求"全面评估 V1 的
    全部页面，设计一些合适的快捷键"；ADR D37；2026-09-23 补 `Esc` 与"帮助要有可见入口"）：
    · **列表页**：`/` 聚焦搜索（原有）、`?` 快捷键帮助、`r` 刷新、`t` 切主题、`n` / `p` 上一页·下一页、
      **`f` 只看收藏 / `h` 回收站**（两个**视图**开关，对应工具栏那两枚 chip，再按一次切回；
      2026-09-23 补，用户问"收藏夹 回收站 有快捷键吗"——它们此前只能靠 Tab 够到，而从列表深处
      够工具栏与够选择条是同一类问题。注意与 `s` 的区别：`s` 是**行**的收藏开关，`f` 是**视图**）、
      **`b` 跳到选中操作条**（批量动作的键盘入口，2026-09-23 补，见下）、
      **`Esc` 清空选择**（有选中时；逐级退出，与 V2 的 `keys.js` 同一条取向，可见等价物是选择条上的
      「取消选择」）；**预览框**：`c` 复制、`d` 下载、`e` 编辑（仅文本、且未超限时按钮可用）；
      **编辑态**：`Ctrl/⌘+Enter` 保存、`Esc` 退出编辑；**对话框**：`Esc` 关闭（原生）。
    · **`b` 为什么必要**：选中操作条在 DOM 里位于**表格之前**（它占的就是头栏那条带子的位置），
      而从列表深处的某一行往回走只能用 Shift+Tab 一站一站退 —— 1440 档实测：第 6 行的复选框到
      带子第一枚按钮 **41 站**（一页 50 行 × 每行 7 个可聚焦控件），于是"勾了几条之后要批量处理"
      对键盘用户事实上不可达。`b` 直接把焦点送过去（`list.focusSelectionBar()`），落点**不停在
      销毁性那枚上**（活跃视图是「复制选中」、回收站视图是「恢复选中」；与 `preview.js` 的初始焦点
      同一条取向：多按一次 Enter 不该删东西），没有选中时不接管。它是**导航键**（只移动焦点），
      与 `↑↓` / `Home`·`End` / `Tab` / `Shift+方向键` 同属"帮助浮层里列、不要求在按钮上写键"那一类。
    · **两条硬前提**：① **输入处一律让路**、② **对话框打开时列表页的键让路**（`dialog[open]` 直接
      退出派发）。少了它们，"加了快捷键"就等于"把页面弄坏"。两条都由探针的负判据钉住。
      ⚠️ 前提 ① 的判据是 **`isTextEntry()`（真的能输入文字的控件）**，不是 `tagName`（2026-09-23 修）：
      复选框也是 `INPUT`，而"用键盘选行/方向键导航"的落点正是它 —— 按 `tagName` 让路会让
      `t`/`?`/`n`/`p`/`r` 在那一刻**全部静默失效**（实测：焦点在行内复选框上按 `t` 主题不变、
      按 `n` 不翻页；换到行内按钮上同一按键立刻生效）。`select` 的字母键是控件自己的选项跳转，仍让路。
    · **不做**：行级导航（`j`/`k`）与**任何销毁性单键**。行选择依赖既有焦点模型与选择集语义
      （`next-target.js`、§3.3 第 33 条），改它要重验一整套焦点交接；而单键删除会把"手滑"变成
      "删掉一条记录" —— `confirm.js` 的初始焦点落在「取消」是同一条安全取向。
    · **发现**：hover 提示里就是键（`title` 形如"刷新列表（r）"），另外工具栏有一枚 **「?」按钮**
      （`aria-label="键盘快捷键"`）与 `?` 键都能打开帮助浮层（2026-09-23 补：此前只有键、没有按钮 ——
      而这份列表存在的理由恰恰是"让不知道自己能按什么的人发现它们"，触屏用户更是永远看不到）。
      帮助浮层与派发器读**同一份表**（`main.js` 的 `listShortcuts()`）⇒ 不会出现"帮助里写了、
      实际没绑"。键帽用 `<kbd>`（`.shortcuts__keys`）。
    · **键 → 按钮的映射靠 `data-action`，不靠显示文案**（2026-09-23 修）：预览框的 `c`/`d`/`e`
      此前用 `textContent.startsWith('下载')` 找按钮 —— 改一次措辞就会让键静默失效；行内那套
      （`list.js`）一直用的是 `data-action`，现在两处一致（`c` 的候选是 `copy` / `copy-image`）。
    · 判据：探针 `KEYS`（`?` 帮助列全**四组** / `t` 真的换主题且可逆 / `r` 真的重发列表请求 /
      `n`·`p` 真的翻页 / **输入处按 `r` 不刷新** / **对话框开着按 `r` 不刷新**）；
      `PRVEDIT` 另钉编辑态的 `Ctrl+Enter`、`Esc` 与两枚按钮的 `title` + `aria-keyshortcuts`；
      `KBD2` 钉上面那三处修复（复选框上的列表级键 / 翻页后的焦点归属 / `Ctrl`+滚轮不被吞 /
      帮助的可见入口 / `e` 走 `data-action`）。

37. **键盘可用性：快捷键只是一半，另一半是"只用键盘能不能把整页用完"**（2026-09-22 用户：
    "你还要详细点捋一下现在的键盘操作都合理完善吗 达到了可用的水平吗 不仅仅只是快捷键"）。
    逐项审计（真实浏览器实测）与结论：
    · **Tab 顺序**：第一个可聚焦元素是跳链「跳到主内容」；之后是顶栏 → 工具栏 → 表头排序 →
      `/ui_v2` 之外的整表 → 分页。一页 50 行的可聚焦控件多达 **374** 个，逐行 Tab 不现实 ——
      这正是行间方向键存在的理由（纯增量，Tab 顺序一个不动）。
    · **焦点可见**：抽测 8 类控件（行复选框 / 行内动作 / 收藏开关 / 排序按钮 / 刷新 / 主题 / 分页 / 跳链）
      全部有可见焦点环（分页那枚"上一页"在第 1 页是 `disabled`，本就不可聚焦，不是缺陷）。
    · **行内动作键**（新增，见第 36 条的表）：`v` 预览、`c` 复制、`d` 下载、`s` 收藏、`i` 置顶、
      `Delete`/`Backspace` 移动到回收站或彻底删除（**仍过确认框**，初始焦点在「取消」）。
    · **`Shift+方向键` 扩展选择**（新增）：与鼠标 `Shift+点击` 同一套语义、共用同一个锚点
      （`anchorIndex`）；普通移动会把锚点跟着走（与"最后点过的那一行"同一条纪律）。
      此前键盘用户只能一条条按 Space —— 勾 50 行就是 50 次。
    · **吸顶链不吃焦点**（修复）：用 Tab / 方向键把焦点送到某一行时，浏览器的滚动是**最小**的
      ⇒ 目标停在 `top=0`，而吸顶链（头栏 + 表头）到 92（折叠）/ 148（展开），焦点环整个被压住。
      修法是给行内控件加 `scroll-margin-top: calc(var(--header-h) + 100px)` —— **按展开态取常数**：
      向上聚焦引发的滚动会把顶栏带回来（滚回顶部附近就展开），按 `--header-h-effective` 算会少 56px，
      实测停在 100 而顶栏已展开到 148，照样被挡。
    · **输入处让路**：焦点在搜索框时按 `v`/`Delete` 只是打字（探针负判据钉住）。
    · **对话框的焦点交接**：预览打开时初始焦点在正文框、关闭后**回到触发它的那枚按钮**（原生
      `<dialog>` 的行为，实测成立）；确认框初始焦点在「取消」（第 9 条）；帮助浮层焦点落在对话框本身。
    · **有意不做**：`aria-sort` 已有（排序表头在 `th` 上写 `ascending`/`descending`/`none`）；
      自定义 tooltip 仍不接收指针事件、也不做键盘触发（完整文本本来就在 DOM 里，键盘出口是行内「预览」）。
    · **重建正文/页脚时必须保留外部节点**（修复，2026-09-22）：预览框用 `replaceOwn()` 而不是裸
      `replaceChildren()` —— 有对话框开着时提示条宿主 `#toasts` 会暂住在页脚里（第 35 条），整块重建
      会把它连同正在显示的提示一起从 DOM 摘掉，此后**所有提示都写进游离节点、永远看不见**
      （用户实测报的是"已保存为新记录的 toast 为什么不会消失"）。`toast.js` 另加一行自愈：
      宿主不在文档里就先接回 `body`。
    · 判据：探针 `KBD`（行内动作键 / `Shift+↓` 扩选与净零 / `Delete` → 确认框且焦点在「取消」且行数不变 /
      `c` 真进剪贴板 / 焦点滚动不被吸顶链挡 / 焦点归还 / 跳链在最前）+ `SAVE`（保存的**真实路径**：
      真发 POST、记录数 +1、弹出**真提示条**且文案匹配、`toasts.show` 那类"方法不存在"的回归、
      以及探针**自己收拾干净**（软删 + 彻底删除，回收站里也不残留））+ 既有的 `KEYNAV`（方向键落到同一控件）。

38. **只用键盘走一遍整页（真实按键的逐站记录，2026-09-23）**。用户追问"你有没有真实的看看键盘操作
    能不能正确且完善地操作页面"—— 本轮用 CDP **真实按键**（`Input.dispatchKeyEvent`，只有
    Tab/Shift+Tab/Enter/Space/方向键/Esc 与已声明的快捷键）从前 60 个 Tab 落点走到各类控件的激活，
    结论分两半：

    **走通了的（实测读数）**：
    · **Tab 顺序**：跳链「跳到主内容」→ 顶栏 4 枚 → 搜索框 → 7 枚类型/范围 chip → 两个 `select` →
      刷新 → 全选本页 → 5 个排序表头 → 行内控件（每行 7 个）→ …。前 60 站**每一站都有可见焦点环**。
    · **跳链真的跳**：Enter 之后 `#main`，**下一次 Tab 落在搜索框**（不是回到跳链）。
    · **排序表头**：Enter ⇒ `aria-sort` `none → descending` + URL `?sort=type`。
    · **类型 chip**：Space ⇒ `?types=File` + `aria-pressed` 跟着走。
    · **搜索框**：打字 ⇒ `?search=ab`；**Esc 清空搜索并清掉 URL**（既有行为，实测确认）。
    · **分页**：Enter 在「下一页」⇒ `?page=2`，焦点**留在那枚按钮上**。
    · **预览框**：打开后焦点在正文框（可滚动），Tab 依次走页脚各枚（回绕时会有一站落在
      `<body>` —— 那是 Chrome 对模态 `<dialog>` 的原生回绕行为，不是焦点逃逸：框外的控件拿不到焦点），
      Esc 关闭且**焦点回到触发它的那枚按钮**。
    · **确认框**：初始焦点「取消」，Tab 顺序 [取消, 确认]，Esc 取消后**行数不变**。
    · **帮助浮层**：焦点落在对话框本身，Tab 在框内，Esc 关闭。
    · **编辑态**：焦点在 `<textarea>`，Tab [正文框, 取消, 保存]，Esc **只退编辑、不关框**。

    **走不通、本轮修掉的六处**（每一处都有实测读数，探针 `KBD2` 逐条钉住）：
    ① **焦点在行内复选框上时列表级键全哑** —— 判据按 `tagName` 让路，而复选框也是 `INPUT`；
    实测 `t` 不换主题、`?` 不开帮助、`n` 不翻页，换成行内**按钮**同一按键立刻生效 ⇒ 改用
    `isTextEntry()`（真的能输入文字的控件）。这条尤其要紧：方向键导航的落点正是复选框。
    ② **`n`/`p` 翻页后焦点掉回 `<body>`**（实测 `activeElement` 是 `BODY`、`?page=2` 已生效）——
    翻页重建整张表，焦点随节点丢 ⇒ 此后方向键与行内键全部失灵。现在翻页前把焦点交给分页条上
    对应的那枚按钮（= 鼠标点它的等价动作）。
    ③ **`Ctrl`+滚轮被预览框的滚轮转发吞掉**（实测 `defaultPrevented=true`）⇒ 预览开着时页面缩放失效。
    ④ **帮助浮层此前只有键、没有按钮**（工具栏新增一枚 `?`，`aria-label="键盘快捷键"`）。
    ⑤ **锚点跨页残留**（实测：第 1 页锚第 6 行 → 翻到第 2 页按 `Shift+↓`×2，选中的是页内第 4–6 行）
    ⇒ `update()` 里按**首行 key** 判"这还是不是同一份列表"，换了就清锚点。
    ⑥ **预览框的 `c`/`d`/`e` 靠显示文案找按钮** ⇒ 改措辞即静默失效，改用 `data-action`。

    **批量动作的键盘入口（同日补齐）**：走查发现选择条上的批量按钮（复制选中 / 批量收藏 / 批量置顶 /
    批量移动）在 DOM 里位于**表格之前**，故从列表深处的某一行只能用 Shift+Tab 往上够（1440 档实测：
    第 6 行的复选框到选择条第一枚按钮 **41 站**）。原先把它登记成"已知限制"，用户追问"为什么不做"
    ⇒ 补上 `b` 键（把焦点送到选择条第一枚**非销毁性**按钮，见第 36 条），并把 `Esc`（清空选择）也
    一起补进表里。两条都由探针 `KBD2` 钉住（`b` 之后焦点在带子里、且不是 `btn--danger-solid`；
    `Esc` 之后选中数归零）。

39. **行内动作的载荷必须现读行的可变引用（`ref.item`），不能用 `buildRow` 的构建期快照**
    （2026-09-23 实测补，`progress.md` §173）。两件事要分开：
    · 按钮的**形态**（类型 / 有无数据 / 是否已删 / 文案）可以用构建期快照 —— 那些字段都在内容签名
      `signature()` 里，变了会重建这一行；
    · 动作的**载荷**必须在点击当下读 `ref.item`：行内开关（收藏 / 置顶）成功后就地换掉 `ref.item`，
      而 `signature()` **不含 `version`** ⇒ 这一行不会被重建 ⇒ 闭包会一直抓着旧快照。
    后果是**自伤式**的：`touchAccess` 回显过期 `version`，服务端按上游 `shouldUpdate`
    （5 分钟内要求 `newVersion >= oldVersion`）判 **409**，而 `touchAccess` 对 409 的既定语义是
    **静默丢弃** ⇒ 复制/下载之后「访问」列不更新（同一个原因让 V1 探针的「零失败请求」判据红）。
    实测：先按 `s` 收藏、再点「复制」，那条 `PATCH` 就是 409；改成 `ref.item` 后同一序列 200。
    同一条纪律的另外两个落点早已写着：`buildCheckbox`、行体点击（都用 `ref.item`）。
...
| POST | `/ui/api/history/clear` | 清空历史：`{scope:'trash'\|'all'}`。trash = 先删已删除的行（**单语句 `DELETE … RETURNING Type, Hash`**，与 `clearAll` 同一条 F5 纪律：读到的集合必须就是被删的行）再按集合清扫它们的数据目录（2026-09-22，ADR D29：回收站里是真数据，只删行等于把字节留给孤儿阶段）；all = 与协议 `DELETE /api/history/clear` **共用** `historyOps.clearAllHistory`（先删行，再按 `clearAll` 返回的**实体集合**删工作目录——最坏漏删孤儿目录，不会误删并发写入的新记录）。**不逐条广播**（上游的广播触发点清单里没有 clear，见 §6 的说明；跨标签页收敛靠 `/ui/api/poll` 的计数变化） | 400 / 415 |
...
| GET | `/ui/api/statistics` | 官方统计 + 按类型分布。**三个计数键口径不同**：`byType` 随 `?deleted=true` 走（工具栏的类型计数要与当前视图同源），`byTypeActive` **恒为活跃口径**（与请求视图无关；**当前没有前端消费方** —— 统计条 2026-09-17 起不再列类型明细，保留它是接口契约与 `test/ui.test.ts` 在钉。⚠️ 2026-09-22（ADR D29）起「已删记录不占 R2」不再成立：软删保留数据 ≤30 天，而「存储占用」的字节数来自 R2 实列（含回收站字节）⇒ 它与这个按类型计数**口径不同**，是有意的）；`starredCountActive` / `starredCountDeleted` 是**按视图各一个的收藏计数**（都是全表聚合、与请求的 `deleted` 无关，两个一起给）——统计条「已收藏」那一格与工具栏「收藏」筛选同屏，卡片用**全库**口径（协议 DTO 的 `starredCount`，含回收站里的行）会出现"卡片说 12、点开筛选只有 9"（2026-09-21 dogfood 实测） | 400 参数非法 |
...
| GET | `/ui/api/info` | 部署信息（客户端该填的地址、版本、传输、保留策略、存储；`cleanup` 为清理状态：`lastRunAt`（**尝试开始**）/ `lastCompletedAt`（**跑到收尾**，2026-09-26 加）/ `lastError` / 各阶段游标） | — |
...
4. **删除是软删，数据文件保留到"真的没了"那一刻**（2026-09-22 改，ADR D29）：`PATCH {"isDelete":true}`
   ...
   **协议面同样如此**：`POST /api/history` 带 `isDeleted=true` 也只标记删除
   （上游 `HistoryService.cs:328/387` 那两处「IsDeleted 就删目录」的等价逻辑已去掉，见 §10 登记）。
   ...
   **回收站视图**（`deleted=true`）：所有记录都可恢复；「彻底删除」是"立刻清掉"的出口（行 + 数据目录）。
   注意两个期限不是同一个数字：活跃记录的保留期是 `HISTORY_RETENTION_MINUTES`（**默认 0 = 不限制**，
   对齐上游 3.3.0；过期的未收藏/未置顶记录会被软删，但默认没有过期这一说），30 天是**已删除记录**的硬删期限。
...
8. **界面自己的复制 / 下载**会**推进 `LastAccessed`**（2026-09-22 用户定案「方案 B」，ADR D32；
   ...
   - **载荷还必须现读行的可变引用**（`ref.item`）：行内按钮若抓着 `buildRow` 的构建期快照，回显的就是
     **过期 `version`** —— 行内开关（收藏/置顶）会就地换掉 `ref.item`，而内容签名里没有 `version`，
     这一行不会被重建 ⇒ 服务端按 `shouldUpdate` 判 **409**、触碰被下面的"静默"吃掉，「访问」列不更新。
     2026-09-23 实测：先按 `s` 收藏再点「复制」就是这个形状（V1 探针的「零失败请求」判据同时红）；
     改成 `ref.item` 后同一序列 200。约定见 §3.3 第 39 条、`progress.md` §173。
...
**编辑框的高度 = 正文区同一套规则**（2026-09-22 用户两问后定，`progress.md` §150.9）：
进编辑时取**正文区此刻的高度**（短文本就是内容高度 ⇒ 两态同高），之后**随输入长高**、
封顶与 `.dialog__body` 的 `max-height: min(64vh, 620px)` **同一个值**（长文本两态同高，都在上限）。
高度由 `preview.js` 的 `fitEditor()` 给（`el()` 的 `style` 必须传**对象**，传字符串会在严格模式下抛
`Failed to set an indexed property` —— 实测踩到过），CSS 里不再写 `height: 40vh`：那是 D31 的
"理想高度"，与正文区是两套规则 ⇒ 实测 11 000 字符记录进编辑时对话框从 724px 缩到 508px。
⚠️ **顶到上限后不每次按键都量**（量一次 `scrollHeight` 的代价实测 500 KB ≈ 60ms），改为**停止输入
200ms 后重量一次** —— 于是"把长文删短到能放下"时框会缩回去（实测 576 → 90px，对话框 752 → 266px），
而连打时那个定时器被反复重置、不触发。

**保留策略表单的排版**（2026-09-22 用户两条要求）：两个数字字段**竖排**（`.field-stack`），
每个字段是**标签在左、输入框在右**一行（`.field-inline` —— 它此前是 `flex-direction: column`，
名不副实，两栏因此各占两行）；且**输入框左缘与上方汇总行的值列对齐** —— 判据是两处共用同一个列宽令牌
（`.field-inline` 与 `.kv__row` 都用 `grid-template-columns: var(--kv-k, 92px) minmax(0, 1fr)`，
实测值列与两个输入框的 `x` 都是 413）。保存键留在行内、与输入框并排（`.panel__form` 的行方向）。
说明文字（`.note`）里另写明**豁免**：「收藏与置顶的记录不受这两项清理影响」（2026-09-22 用户问
「收藏和置顶的会不会被清理」后补的 —— 服务端两条软删查询本来就带 `Stared = 0 AND Pinned = 0`，
与上游同名谓词一致，但界面上原本一个字都没说；**看不到就等于这个承诺不存在**）。
判据是探针的 `RETENTION-NOTE`（找到那段文案、且它在对话框里**真的可见**）。
...
| 选中操作条出现（`selection-in`） | **从「没有选中」到「有选中」那一次**（容器不重建：`renderHead` 只 `replaceChildren` 子节点 ⇒ 再选一行 / 切筛选 / 翻页都不重放） | 只碰 `opacity` + 4px `translate`（不动布局、不影响吸顶）；`prefers-reduced-motion` 下直接出现 |
...
| 顶栏折叠（`header-fold`） | **滚动状态变化**：向下滚且离开顶部 120px ⇒ 整条滑出；向上滚 / 回到顶部 ⇒ 滑回（**与选择集无关**：多选时不弹回来，见 §3.3 第 34 条） | 位移走 `transform`（合成层、不动布局）；状态由 `html[data-header]` 表达（**只写属性，不每帧改样式**）；`prefers-reduced-motion` 下瞬时到位；省下的 56px 由 `--header-h-effective` 让给头栏与表头 |
...
| **列表加载失败** | 首次加载与**新查询**失败都显示持续的「加载失败 + 原因 + 可行时重试」，新筛选下不会继续显示上一份查询的旧行；同一查询的后台刷新失败仍保留旧行并提示。筛选/翻页/排序改变时，旧行仅在请求中短暂变淡作占位，表格与批量操作条同时 `inert`，失败后收起（D38） |
| **失去联系**（列表/轮询/统计失败） | 显示 `role="status"` 横幅；三条请求链各自登记和清除失败来源，统计或轮询成功不能抹掉仍失败的列表状态，直到对应请求成功或收到正常的 4xx 回答（D38） |
| 回收站为空 / 收回筛选 | 空状态按语境换文案与出口：回收站给「返回历史记录」，筛选态给「清除筛选条件」（含时间范围与回收站两个新维度） |

### 9.4 深色模式

- ✅ 令牌层整体重映射（组件层一行未改，符合「level-2 token remap only」）+ 首帧前定主题（实测 5 次重载 `data-theme` 均在首帧前就位、CLS 全 0 —— ⚠️ **那一次量的是主题重载**：首屏 CLS 是另一件事，见 §11 的探针读数，V1 实测 ≈0.006）。该脚本**本轮从内联外置**为 `/ui_v1/js/theme-init.js`：外链才能让 CSP 保持 `script-src 'self'`（内联要么开 `'unsafe-inline'`、要么维护 hash）。它必须是**经典脚本**——`type="module"` 默认 defer，会晚于首帧。
...
| 10 | 连续重载 5 次 | 主题每次都在首帧前就位、**主题重载**的 CLS 全 0（首屏 CLS 是另一件事，见 §11 的探针读数）、零 console 错误 |
...
- **触屏平板（>860px + `pointer: coarse`）不是窄屏**：卡片重排只在 ≤860px 生效，所以平板仍是表格；
...
**另一类预算：首屏 CLS**（两个探针的 `PERF` 组自动判定）—— **≤ 0.1**（"good" 阈值）。它的职责是
**抓回归**（修前那两条是 0.85 / 0.93），不是把当前值钉死：V1 实测 ≈0.006、V2 宽屏 0.0026 / 窄屏 0.0078
（2026-09-22 修掉 `.overview__ghost` 的 6px 与窄屏趋势图的 0→26px 之前，窄屏是 **0.1217**）。
⚠️ **窄屏是另一套布局**（概览带折两行、趋势图独占一行）：只在 1440 档量会漏掉只在窄屏越预算的缺陷，
故 `AGENTS.md` §2 第 5 条要求 V2 跑 **1440 与 390** 两档。CLS 之外的占位纪律见 §8 与
`docs/ui-v2-design.md` §5。
...
| ~~入场错峰**只在首屏**播（此后按行对账）~~ | **2026-09-22 已整条移除**（用户要求）：原依据是"级联的尾巴（12 行错峰、下标 0–11 ⇒ 11 × 40ms = 440ms）读起来像内容慢半拍"—— 那条依据直接导致删掉整个动画，而不是只限制它播的时机（判据见 `progress.md` §155） |

...
30. **行内徽标（置顶 / 数据不可用 / 含数据文件 / 长文本）靠内容列的右缘站成一列；内容格 ≤360px 时
    徽标让出正文行**（2026-09-22 定案，ADR D35，`progress.md` §148）：
    · **紧贴正文是错的**：那样徽标落在"正文结束的地方"，同一屏里位置各不相同（1440 实测：
      短正文行的「置顶」在 x=500、长正文行的「置顶+长文本」在 x=705，差 205px）⇒ 一屏之内位置
      各异的徽标**没法扫读**，"哪几条是置顶的"要靠眼睛逐行找，而那正是徽标存在的意义。
      现在 `margin-left: auto` 把它们推到右缘 ⇒ 徽标成一列，正文照旧按需收缩（`gap` 仍是最小间距）。
    · **窄到放不下时，徽标让位**：判据是**内容格自己的宽度**（`@container (max-width: 360px)`），
      不是视口 —— 同一视口下内容格的宽度由列布局决定（实测 900px 视口的内容行只有 348px，
      比 720px 的 556px 还窄），媒体查询写不出这条判据。低于 360px 时正文只剩个位数
      （390px 视口实测 97px ≈ 7 个汉字）⇒ 徽标 `flex-basis: 100%` 独占下一行，正文拿回整行
      （实测 430px：137 → **266px**）。
    · **只在卡片档（≤860px）换行**：表格档的行必须**等高**（`.cell-content__text` 那条
      "数据表的等高栅格比多显示一行更重要"）—— 900px 视口是表格档而内容格只有 348px，
      不设这道门会让那一档的徽标行 47 → 67px，同一张表两种行高。
    · 代价如实记：卡片档里带徽标的行高 28px（103 → 131px，粗指针 117 → 145px）；骨架行画不了
      "哪一行带徽标"，仍按基础值估（§9.3 的 loading 行）。
    判据：`components.css` 的 `.cell-content`（`container-type`）/`.cell-content__flags`
    （`margin-left: auto`）/`@media (max-width: 860px) { @container … }` + `row-content.js` 的 `buildFlags`。
...
> ⚠️ **一处例外的隐式共享面（2026-09-22 发布前审核第 11 轮查实）**：两版**共用同一个
> `localStorage` 主题键 `sb-ui-theme`**（V1 `js/main.js` + `js/theme-init.js`；V2
> `js/theme.js` + `js/theme-init.js` 的 `THEME_KEY`）—— 于是"在 V1 切深色、进 V2 也是深色"。
> 这是**有意的**（同一个人的两个界面不该各记一套偏好），但它确实是模块层之外的一条共享通道，
> 故在这里点名。V2 另有 `sb-ui-density`（紧凑行高，V2 独有），**V1 不读它**、也不该读
> —— 密度是开发测试版自己的显示偏好（V1 的行高由 `components.css` 的等式钉死，见 §9.3）。
> 改任一侧的键名/取值集合时，另一侧要一起看：这是**唯一**一处不能只改一版的共享状态。
...
| Service Worker / 离线可用 | **不做**（2026-09-22 发布前审核第 15 轮补登记）。这个界面是"服务器上现在有什么"的读侧面板：数据全在 D1/R2，本地缓存没有可读的东西；而缓存一份列表反而会让"我刚在手机上复制的内容"看不见（那正是本页最该立刻反映的事）。`manifest.webmanifest` 的 `display: standalone` 只为「添加到主屏幕」这一个便利，**不代表**离线可用 |
| 打印样式 | **不做**。这是数据表 + 对话框的操作面板，没有打印场景；要把某条内容带走，出口是行内/预览里的复制与下载（都已在位、且带成功反馈） |
| 界面上的"新建 / 上传记录"入口 | **只提供预览里的「编辑」**（保存 = 新建一条**文本**记录，ADR D30）；没有"粘贴一段新文本"「拖文件上传」这类通用入口 —— **写入的入口是官方客户端**，本界面是它的读侧面板（空状态那句「在任意设备上复制内容后…会同步到这台服务器」就是这条约定）。要改这条约定就得先回答"多终端冲突怎么判"，那是另一个决定 |
...
| 状态 | 实现 |
|---|---|
| loading | 骨架屏占位（保留布局，不跳）。**2026-09-18 之前这一行只是一句声明**：`list.js` 根本没有加载档，而 `boot()` 的顺序是 `render()`（items 还空）→ `refresh()`，于是首屏那次请求落地之前的整段时间里，界面画的一直是空状态 —— 库里有记录时它是一句假话（详见 `progress.md` §85）。现在由 `store.loading` 驱动：**一个表头占位 + 行数按当前页大小**，行高、行距与真实行逐项同源（表格档 47px、卡片档 103px、粗指针 117px；**带徽标且内容格 ≤360px 的卡片行实测 131px / 粗指针 145px** —— 骨架画不了"哪一行带徽标"，仍按基础值估，2026-09-22 起这条换行由 `@container` 确定性触发，见 §3.3 第 30 条；容器的 `padding`/`gap` 归零、行与真行一样是「方角 + 1px 分隔线」；表头占位 47px、粗指针 61px —— 2026-09-22 发布前审核第 5 轮补齐表头与行距，见 `progress.md` §137），数据落地时折线以上**零位移**（三档实测：骨架首行与真实首行 y 相同、分页 y 也相同） |
...
- **为什么必须重排**：按桌面列宽摊分，390px 下内容列只剩 **84px**（约 5 个汉字/行），
  ...
  桌面设计的截图」那种失败。
  重排后实测（2026-09-22 复测，headless Chromium + 触屏模拟，`?types=Text&pageSize=20`，
  量的是 `tr.row` 的高度与 `.row__cell-content` 的宽度）：行高 **117px**（每屏约 6 张卡片），
  操作按钮 **44×44**，内容列在 360/375/414/480/600/720 为 **154 / 169 / 208 / 274 / 394 / 514**，
  文档横向溢出 0、越界按钮 0。⚠️ 两处与旧记录不同，都是**布局改过之后**的必然结果：
  ① 旧值 184/224/239/278/344/464/584 与「行高 106px」是 2026-09-18 的行布局，此后的固定槽位、
  粗指针列宽（`--col-star-coarse` 112px）都改动过它；② ≤340px 的**粗指针**下会出现一次额外换行
  （勾选框 44 + 内容 140 的下限 + 收藏列 112 > 卡片内宽 264 ⇒ 收藏列单独一行），
  那时内容列反而比 360px 更宽（210 vs 154）、卡片高 165px —— 溢出仍为 0，只是"严格单调"
  这条性质从 **360px 起**成立（fine pointer 下 320px 起就成立：156/226/316/396/556 @320/390/480/560/720）。
  早先让元信息单独成列时出现过 414 比 375 更窄的反常，那类问题仍然不存在。
...
| 窄屏行布局（触屏模拟，2026-09-22 复测） | 内容列 84px → **154@360 / 169@375 / 208@414**、行高 170px → **117px**（固定槽位后的值；旧记录 106px 是 2026-09-18 的行布局）、操作按钮 **44×44**（独占整行、换行确定）、元信息「类型 · 时间」可见、表头排序保留、溢出 0 |
| 桌面（1440，2026-09-22 复测） | 行 **47px**（= `.table td` 的 `8+8+1+30`；旧记录 55px 是更早的密度）、七列齐全、`.cell-content__meta` 隐藏——窄屏改动对桌面零影响 |
| 触屏命中区（`pointer: coarse`，2026-09-22 复测） | `.btn` / `.select` / `.segmented__item` / `.check-wrap` / `.th-sort` / `.icon-btn` **全部 ≥ 44px**（实测 44 / 44 / 44 / 44×44 / 44×44 / 44×44 —— 分段控件此前记成 40px，实际由 `--hit-min` 兜住） |

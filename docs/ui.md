...
| `js/components/shortcuts.js` | **快捷键帮助浮层**（`?` 打开）+ 目录渲染：三组（列表页 / 预览框 / 编辑正文）逐条列键帽与说明。目录与派发器读**同一份表**（`main.js` 的 `listShortcuts()`）⇒ "帮助里写的"与"实际绑的"不会漂移（取向见 §3.3 第 36 条、ADR D37） |
...
35. **对话框开着时的提示条：把 `#toasts` 宿主搬进最上层对话框，而不是在框里另造一条**（2026-09-22，
    ...
    · **落点**：宿主挂进 `.dialog__foot`，CSS `position: absolute; bottom: 100%` ⇒ 浮在页脚**之上**、
      水平居中，不参与对话框的列布局、也不盖住页脚按钮；框 `close` 时宿主放回 `body`
      （宿主仍是 `position: fixed`，视觉落点不变，剩下的提示条不会跟着关闭的框一起消失）。
    · 副作用（正面的）：`.toast__action`（"重试"那类动作按钮）在对话框开着时**才**真的点得动。
    · 判据：探针 `PRVTOAST`（同宿主 / 临时打开命中区后**真实鼠标**点中它自己 / 浮在页脚之上 /
      2.6s 自收 / 关框后宿主回 `body`）。

36. **快捷键：少而稳的七条，每条都必须有可见的等价按钮**（2026-09-22，用户要求"全面评估 V1 的
    全部页面，设计一些合适的快捷键"；ADR D37）：
    · **列表页**：`/` 聚焦搜索（原有）、`?` 快捷键帮助、`r` 刷新、`t` 切主题、`n` / `p` 上一页·下一页；
      **预览框**：`c` 复制、`d` 下载、`e` 编辑（仅文本、且未超限时按钮可用）；**编辑态**：
      `Ctrl/⌘+Enter` 保存、`Esc` 退出编辑；**对话框**：`Esc` 关闭（原生）。
    · **两条硬前提**：① **输入处一律让路**（`INPUT`/`TEXTAREA`/`SELECT`/`contentEditable` 与 IME
      组字期间）；② **对话框打开时列表页的键让路**（`dialog[open]` 直接退出派发）。少了它们，
      "加了快捷键"就等于"把页面弄坏"。两条都由探针的负判据钉住。
    ...
    · **发现**：hover 提示里就是键（`title` 形如"刷新列表（r）"），另外 `?` 打开帮助浮层；
      帮助浮层与派发器读**同一份表**（`main.js` 的 `listShortcuts()`）⇒ 不会出现"帮助里写了、
      实际没绑"。键帽用 `<kbd>`（`.shortcuts__keys`）。
    · 判据：探针 `KEYS`（`?` 帮助列全三组 / `t` 真的换主题且可逆 / `r` 真的重发列表请求 /

...
| `js/components/info.js` | 部署信息对话框（分**客户端配置 / 服务器 / 存储 / 保留策略 / 清理任务 / 数据完整性 / 危险操作**七段，键值两列、窄屏上下排布，页脚有「关闭」）；服务器地址一键复制用原地成功态；保留策略表单带服务端同值的上界校验，**显示口径与 `cleanup.ts` 的内置默认同值**（`null` = 内置默认 7 天 / 1000 条、`0` = 关闭该阶段，分钟数按 分钟/小时/天 分档），见 `progress.md` §63.1 |
...
19. **表单错误挂到字段上**（2026-09-18，`components.md` §2 的 error 格）：两个表单
...
7. **置顶恒优先（`Pinned DESC` 恒在主排序列之前）**：这个接口没有"默认排序"这一档（`sort` 恒有值），
   ...
   而不是等下一次轮询时自己跳走（`public/ui_old/js/main.js` 与 `public/ui/js/boot.js` 的同一条判据）。
...
`handfeel.md` §7 的"跟随"那一族（相机/光标/导轨）在 V1 里**没有消费者**：`public/ui_old/js` 里

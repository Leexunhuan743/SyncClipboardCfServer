...
> **2026-09-18 新增的一处跨目录依赖**：面向用户的**文案**（删除确认的语义句、列表错误的人话翻译、
> 剪贴板失败原因）改为两版共用同一份 —— `public/ui/js/messages.js`，V1 经
...
| 文件 | 职责 |
|---|---|
| `index.html` / `login.html` | 页面外壳与挂载点；主题在首帧前由阻塞式的 `/ui/js/theme-init.js` 定好（深色用户不会看到白闪）；列表页的 `modulepreload` 清单里含**共用文案表**（`/ui/js/messages.js` 与它依赖的 V2 `/ui/js/format.js`） |
...
| `js/components/header.js` | 顶栏（标识、「复制最近一条」、**「部署信息」（图标即推送状态）**、主题切换、会话操作）。那枚控件 2026-09-18 由"状态胶囊 + 独立的部署信息按钮"合成：**可见文字只有「部署信息」**（按钮的名字就是它做的事），**推送状态只由图标承载**（三态三字形：广播/信号、缺口圆环旋转、刷新箭头），**hover 给「状态词 + 一句解释」**，整枚一个命中区，位置紧跟「复制最近一条」。见 `progress.md` §60 |
...
| `js/components/list.js` | 结果区：表格、行、**四个固定槽位的行内动作**（预览/复制/下载/删除，缺失的动作放等宽占位——同一动作在每行的位置恒定）、排序表头、选择条（Shift 范围选择；批量按钮带**进行中态**）、**一键复位筛选**、空状态；**同一视图内的刷新按行对账**（内容未变的行不重建，见 §3.3） |
...
| `js/components/preview.js` | 预览对话框（文本全文 / 图片原图 / 不可用态）；点背景关闭、打开时焦点落在主操作、长文本先给加载态；主操作文案与行内统一（「复制文本」「复制图片」「下载」） |

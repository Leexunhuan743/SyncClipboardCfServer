...
33. **回收站视图的「清空回收站」常驻在操作带里、「取消选择」的左侧**（2026-09-22 用户三次定形）：
    ...
    · 窄屏（≤560px）它走**已有的那条**规则（`.results__selection .btn:has(svg) .btn__label`）收成图标：
      ...
      折行、把 45px 的头栏顶高。
    · **选中态下这条带子（连同整个头栏）吸顶**（2026-09-22 用户："多选了之后弹出来的那一行也固定在上面"）：
      `.results__head:has(.results__selection-count) { position: sticky; top: var(--header-h); z-index: 9 }`
      —— 判据与底色高亮同一条（「已选 N 条」在不在）：未选中时头栏只是计数与「清除筛选」，
      不值得常占 45px，照旧随页面滚走。表头的吸顶偏移必须跟着让一格
      （`@media (min-width: 861px)` 里的 `.results:has(…) .table th { top: calc(var(--header-h) + var(--results-head-h)) }`），
      否则两条带子会重叠；卡片档（≤860px）表头是随页面滚走的紧凑工具条，不让位。
      实测（1440×900，滚到 y=800）：未选中 `headY=-567`（照旧滚走）、表头 56；
      选中 `headY=56`、`headH=45`、表头 101、无重叠、五枚批量键都在 y=60。
    · 判据：`list.js` 的 `headEmptyTrash` + `renderHead` 的 `showResident`/`headSelection.hidden` +
      `layout.css` 的 `:has(.results__selection-count)` 两条（底色 + 吸顶）+ `tokens.css` 的 `--results-head-h`；

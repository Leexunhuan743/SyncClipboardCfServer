...
>
> 1. **§ 编号是引用锚点，已冻结。** 全仓有 **53 处**具体 `§x.y` 引用，分布在 **24 个文件**、覆盖 **29 个编号**
>    —— 其中源码 **27 处**（`public/ui_v1/js/**` 10、`public/ui_v2/js/**` 16、`src/ui/query.ts` 1）、
>    探针 3 处、文档与记忆 **23** 处。⇒ **不重排、不合并、不删节**；改代码时也不必回来改这里的编号。
>
>    **口径与检索式**（2026-09-20 复算，可照抄复跑）：同一行**同时含本文档路径与 `§x.y`**且不以本文档自身为来源，
>    树 = 归档那一版（`da1ee44`）：
>    `git grep -n -E 'AUDIT-v1-v2-divergence' da1ee44 | sed 's/^da1ee44://' | sed -n '/§/p' | sed -n '/^docs\/archive\/AUDIT-v1-v2-divergence.md:/!p'`
>    ⇒ 处 = 数输出行数；文件 = `cut -d: -f1 | sort -u | wc -l`；编号 = 每行首个 `§x.y` 去重。
>    四组**代码侧**与旧值逐位吻合（16 / 10 / 1 / 3），差额全在"文档与记忆"（旧记 14，实为 **23**）
>    与文件数（旧记 23，实为 **24**）—— 那两栏的旧口径不可复原（疑似在**文件系统**上数的：
>    `.workbuddy/**` 与未跟踪的台账不在 `git grep` 视野里）。旧值 44 / 23 / 28 保留在下方补记里供对照。
>
>    ⚠️ **2026-09-20 补记：把口径写出来，并给出可复算的实数。**（只动横幅，不动报告正文。）
>    上面那组数字是**写下它的那一刻**的读数，与它所在这棵树的实数**已经不一致** —— 同一笔提交
>    （`da1ee44`）在写完之后又补了 `progress.md` §94 的第 18/19 行，而那两行本身就在引用本文。
>    复算口径 = 「**同一行同时含本文档路径与 `§x.y`**」：
>    `git grep -o -h -E 'AUDIT-v1-v2-divergence.*§[0-9]+\.[0-9]+' HEAD -- <范围>`（数输出行数）。
>    按它复算：`public/ui_v1/js/**` **10** ✓、`public/ui_v2/js/**` **16** ✓、`src/ui/query.ts` **1** ✓、
>    `test/manual/**` **3** ✓（四组逐位吻合 ⇒ 这条口径很可能就是原口径）；**文档与记忆 17**（原写 14）、
>    **文件数 22**（原写 23）。后两栏的**口径不可复原**：原数疑似在**文件系统**上数的 ——
>    `.workbuddy/**`（未跟踪）与未跟踪的 `docs/AUDIT-*-diff-6ebcf6e.md` 都不在 `git grep` 的视野里，
>    而它们也在引用本文。`28 个编号`（去重）**本轮未复算**。
>    ⇒ **2026-09-20 收口（用户决定）**：上面那行已按这条口径改成 **53 / 24 / 29**；旧值 44 / 23 / 28 记在
>    本节供对照（"当时抽出几处"没有丢）。`28 个编号`（去重）也已复算 —— 实为 **29**。
>    要引用请用上面那行，口径就是本节这条。

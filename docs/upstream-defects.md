# 上游缺陷与怪癖的复刻清单 · 逐条处置（16 条已复刻 + 5 条未复刻）
...
>
> **与 `upstream-issues.md` 的分工**：那份文件是**给上游提 issue 的稿子**（13 条，只收录上游自身的问题）；
...
| **D · 待办** | 2 | 本轮新发现的**文档缺口**，与 1 条**有意不做**（含理由） |
| **不改但需知** | 2 | 无行为后果，仅记录（判定正确性用） |
...
#### D3 · 有意**不做**：`GET /file/{name}` 的 `%2e%2e` 路径形态（**记录理由，不改**）
...
#### D4 · 待观察：`TextProfile` 的 10 KiB 截断阈值是**硬编码**
...
| 「`GetFileFromFolder` 未调用 `InvalidFileName`」 | 同上（D3）：可达性未实测，按 ADR D10 不作为结论 |

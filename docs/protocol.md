...
| 恢复带数据文件的已删记录（`PATCH isDelete:false`） | **拒绝**：`HistoryService.cs:64`（`IsDelete is false && IsDeleted && TransferDataFile 非空` → `(null,null)`，客户端拿到 404） | **允许**：记录连同数据回到活跃列表 | 上一行的必然推论（数据保留了，恢复就该成功）。官方客户端不会因此走错分支——它只是"恢复成功" |
...
| `SearchText` 的**字节上限** | 无（D1 的 LIKE 复杂度上限只在**转义后**出现，见下） | 入口按字节校验，超 `MAX_SEARCH_BYTES`（48）→ **400**（`serialization.ts:484-490`） | 有意偏离（fail-loud）：超长模式会在真 D1 上以 `LIKE or GLOB pattern too complex` 500（§95 实测）；官方客户端搜索框正常输入远小于此 |

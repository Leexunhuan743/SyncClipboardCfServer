...
```jsonc
{
  ...
  "transferDataHash": null,// 上游 3.3.0 #413：传输数据文件的 SHA-256（64 位 hex，统一大写）；
                           // null 时省略（WhenWritingNull）。**双向**：PUT 是声明值（校验用，不落进
                           // Profile 实体）；GET /SyncClipboard.json 与 RemoteProfileChanged 回传
                           // 库里存的真实值（迁移前的旧记录无值 ⇒ 省略，客户端据此跳过校验）
  "size": 123              // null 时省略
}
```
...
| GET | `/` | 200 文本 `"Server is running."`；**浏览器导航**（`Accept` 含 `text/html`）且界面开启（`UI_ENABLED!=false`）→ 302 `/ui_v1/`（**默认界面**入口；2026-09-18 起直接跳到 `/ui_v1/`，省去一次从 `/ui/` 的跳跃）。客户端从不 GET 根路径（`Test()` 与 `GetFolderSubList()` 都是 PROPFIND），故该分支不影响协议行为 |

> **非协议路径**：`/ui_v1/*`、`/ui_v2/*`（界面静态资源）、`/ui/`（跳转壳）与两版共用的界面接口
> `/ui/api/*` 是本实现附带的 Web 界面，**不属于协议契约**——
...
| 并发 | 单进程信号量串行（`HistoryService.cs:19` 的 `_processSem`，**static ⇒ 仅同进程有效**）；`Update`（`:33-83`）与 `AddProfile`（`:182-209`）都是「读 → 判定 → 写」，`SaveChangesAsync` **无条件覆盖**；DB 层**无** `(UserId,Type,Hash)` 唯一约束（`Migrations/20251105014242_Init.cs:40-43` 只有主键） | ① `(UserId,Type,Hash)` **UNIQUE 索引**（`schema.sql:35`），并发抢先插入转为确定的合并路径（`src/db.ts:151-182`、`src/profile.ts:336-338`）；② `updateEntityIfVersion`（`src/db.ts:222-228`，`UPDATE_ALL_SQL` 单一来源）`UPDATE … WHERE ID=?16 AND Version=?17`，按受影响行数判定冲突 → 409 | **有意义偏离**（不只是"实现不同"）：上游这两点只有在**多进程/多副本共享同一个 `history.db`** 时才显现 —— 无唯一约束 ⇒ 同 hash 插出重复行（此后 `FirstOrDefaultAsync` 不保证取到哪一行，`GetRecentTransferFile` 的逐条回退会让客户端反复取到"另一行"的数据文件）；无版本条件更新 ⇒ 两个并发 PATCH 各自通过 `ShouldUpdate` 后静默覆盖，而客户端 `OfficialAdapter.cs:323-337` 恰恰**依赖 409** 发现冲突并重试。上游有过一次同主题修复（`e79a18d6`「修复：数据库并发问题」，在基线之前），但只在客户端侧处理。逐条对照见 `docs/upstream-defects.md` Q17/D2 与 `upstream-issues.md` Issue 14 |
| 传输数据 SHA-256（上游 3.3.0 #413） | `HistoryRecords.TransferDataHash` 列 + `POST /api/history` 可选请求头 `X-SyncClipboard-Transfer-Data-Hash` + `GET …/data` **回带同头** + 「有数据但取不到」由 404 改 **422**（ProblemDetails `History transfer data is invalid` / `code:"history_data_invalid"`）+ `PrepareTransferData` 对旧记录**惰性回填**哈希 | 同列同头同 422，**错误体同为 ProblemDetails JSON**（`status`/`title`/`code`/`detail`，与上游同构 —— 2026-09-22 逐行复核上游 `HistoryController.cs:72-79` 后确认，此前误记为"本实现纯文本"）；**不做惰性回填** —— 迁移前入库的记录哈希为 `''` ⇒ `/data` **不带**该头、客户端据此跳过校验（上游回填需把对象整体读进内存，代价不成比例；新上传的记录都带） | 兼容性：3.3.0 客户端两个入口都按「可选」处理（头缺失 ⇒ 跳过校验），旧记录与其旧客户端行为不变 |
| `POST /api/history` 的**声明头与文件不符**（上游 3.3.0 #413） | **422** + ProblemDetails `History transfer data is invalid`：`HistoryService.SaveTransferDataAsync`（`HistoryService.cs:449`）调 `Utility.VerifyFileSHA256(filePath, declaredTransferDataHash, token)`，不匹配时它抛 `InvalidDataException`（`Shared/Utilities/Utility.cs:102-106`），被 `:457-461` 包成 `HistoryTransferDataException`，控制器 `:185-192` 映射为 422 | 同左（422 + 同形 ProblemDetails） | **两侧一致，不构成差异**（2026-09-22 逐行复核上游后改正：本行此前写成「上游 400 / 本实现 422」，那是把 `PUT` 路径的状态码误套到了 `POST` 路径上 —— `PUT` 侧上游确实是 400 `Hash is not match data.`，见 `SyncClipboardController.cs:243-250`） |
| `POST /api/history` 的**声明头形状**（上游 3.3.0 #413） | 无 data 却带头 → 400 `… cannot be set without transfer data`；重复值 → 400 `… must contain exactly one value`；空值 → 400 `… cannot be empty`；非 64 位 hex → 400 `Hash must be a 64-character SHA-256 hex string. (Parameter 'hash')`（`HistoryController.cs:169-172`、`:226-242`、`Utility.NormalizeSHA256`） | 前三类同左（逐字）；非 64 位 hex 同左；**空值目前落到 `… must contain exactly one value`**（本实现把空段过滤后再判段数） | 文案级差异（同为 400、客户端不解析错误体）；见下行 |
| 缓存 | `IMemoryCache` + 显式失效，但**失效语句用的 key 与读写的 key 不是同一个**：`SyncClipboardController.cs:120` 是 `_cache.Remove("SyncClipboard.json")`（字面量），而 `:128/130/138/150/154/261` 读写的是 `cacheKey = profilePath`（绝对路径）⇒ 失效**从未生效**；且 `_cache.Set` 无任何过期策略 ⇒ 进程外改动（共享卷、手工修复 `server/SyncClipboard.json`、备份恢复）后长期返回内存旧值 | 无缓存（D1/R2 直读，`src/routes/webdav.ts:69-102`） | **更强一致**（不只是"策略不同"）：上游那两条各自都会造成"看到过期的当前剪贴板"，本实现结构性不存在该故障；代价是每次 GET 一次 D1 读（单用户场景可忽略）。逐条对照见 `docs/upstream-defects.md` Q16 与 `upstream-issues.md` Issue 8 |
| 保留/清理 | `Server.Core/Services/History/HistoryCleaner.cs` 三类后台任务（10min / 12h / 12h），软删单批 500、无批次上限 | `src/cleanup.ts` 的 Cron Trigger **每 20 分钟**批量执行同类语义；软删单批 **500**（对齐上游：`Shared/Utilities/HistoryManagerHelper.cs:19`/`:66` 的 `BatchSize = 500`）、受**子请求预算 + CPU 行字节预算**双重截断并以游标续跑（Cron 与 HTTP 共用同一档 **10 ms CPU 平均预算** —— 偶发越界由 rollover CPU time 吸收、不报错，只有**持续**越界才终止；整轮被终止时**轮首心跳**已落下 `cleanup:lastRunAt`、而**完成戳** `cleanup:lastCompletedAt` 只在轮尾写 ⇒ 界面据此区分「跑完了」与「只开始了」） | 等价（周期 20min vs 10min；批一致；软删/硬删/孤儿判定与广播一致。差异只在"积压收敛速度"与平台预算机制 —— 子请求 800/轮 + 行字节 256 KiB/轮/阶段，见 design.md §9 与 D40） |
| 软删（`isDelete:true`）对**数据文件**的处置 | **立即删除**工作目录：`HistoryService.cs:81` 的 `DeleteProfileDataIfNeed` → `DeleteProfileData`（`IsDeleted` 为真就删）。**三条写路径都是这个语义**：`PATCH /api/history/{type}/{hash}`（`:81` 的 `Update`）、`POST /api/history` 的既有记录分支（`:325` 的 `UpdateExistingRecordDto`）与新增分支（`:329` 的 `AddNewRecordDto`） | **保留**：数据留到"真的没了"那一刻 —— 30 天硬删（Cron 的 hardDelete 阶段，批次清扫）或用户点「彻底删除」/「清空回收站」时才清。**三条写路径一致保留**（`historyOps.applyHistoryUpdate` 与 `profile.addRecordDto` 都不再调 `DeleteProfileDataIfNeed` 的等价逻辑）。推论：`GET /api/history/{type}/{hash}/data` 对已删记录从 404 变 **200**（回收站里的图片因此能预览） | **有意偏离**（2026-09-22，ADR D29）：上游那条语义让"回收站"对**图片/文件**变成单向门。代价是多占 ≤30 天 R2 空间（$0.015/GB/月；「彻底删除」可立刻释放）。对官方客户端**只更好**：恢复后它照常把数据下回来（`RemoteHistoryChanged` 带 DTO，客户端见 `!IsLocalFileReady` 即 `EnqueueDownload`） |
| 恢复带数据文件的已删记录（`PATCH isDelete:false`） | **拒绝**：`HistoryService.cs:65`（`IsDelete is false && IsDeleted && TransferDataFile 非空` → `(null,null)`，客户端拿到 404） | **允许**：记录连同数据回到活跃列表 | 上一行的必然推论（数据保留了，恢复就该成功）。官方客户端不会因此走错分支——它只是"恢复成功" |
...
| 错误响应体 | 显式 `BadRequest("…")` → 400 **`text/plain` 纯文本消息**（**2026-09-22 A/B 实测**：`POST /api/history` 的 `Type is invalid or missing`、`POST /api/history/query` 的 `after must be less than before` 两侧**逐字相同**）；**模型绑定失败**（如 `PATCH` 传 null body）由 `[ApiController]` 自动返回 400 `application/problem+json`（ValidationProblemDetails）；422 为 ProblemDetails 应用 JSON | 400 一律 `text/plain` 消息；422 为**与上游同构**的 ProblemDetails JSON（`status`/`title`/`code`/`detail`，见上调两行） | 唯一差别在**模型绑定失败**那一类（上游 `application/problem+json` ↔ 本实现纯文本）；状态码与显式文案一致，官方客户端只判状态码。实测件是官方 `v3.2.0` 发布件（早于基线 14 个提交），但错误体形态属框架层，不受这区间那两处服务端改动影响 |
...
| 应用层解压上限 | 无 | **有**：Group zip 解压总量 64MiB / 条目 1000 / 单条目压缩比 100:1（比值守卫含 8MiB 绝对下限；见 `src/hash.ts`） | **有意偏离**：客户端对文件夹上传**只有总字节闸门**（`ContentControlHelper.cs:63-70` 的 `ValidateSize`，默认 20 MB，按**解压后**总长）、**没有条目数闸门** ⇒ 「1000+ 个小文件、合计 <20 MB」这种**合法上传**会被拒（POST→422、PUT→400）；上游无此防护（同为全量解压）。64 MiB 总量上限在默认 20 MB 下不可达（cap 随 zip 体积收缩）；100:1 比值守卫另有 8 MiB 绝对下限 ⇒ 只有病态内容才踩到 |
...
| **附件响应头** | 仅 `Content-Type` | 一律 `X-Content-Type-Options: nosniff`；可渲染类型（HTML/XML 家族，**按后缀判定**，含 `+xml`）额外 `CSP: default-src 'none'; sandbox` + `Content-Disposition: attachment`；**内联与下载的完整口径见下方「MIME 表与附件加固」行**（默认-deny 内联白名单） | **有意加固偏离**：附件与 API 同源、浏览器会自动附带已缓存的 Basic 凭据，直接打开可读取全部历史（存储型 XSS）。桌面客户端不读这些头，已 E2E 验证无回归；代价是浏览器不再内联预览 HTML/SVG/XML 附件，且**非白名单类型（压缩包、Office、未知类型）自 2026-09-21 起一律改为下载**（此前它们是无 disposition 的内联，见 `progress.md` §106） |
...
| 401 的**响应体** | 空体（`BasicAuthenticationHandler.cs:33-35` 只写状态码与头） | 纯文本 `Unauthorized` | 官方客户端只判状态码；第三方若解析 401 体会拿到文本而非空体 |
| 未知路径 / 非浏览器 GET `/` 的**鉴权** | 路由不匹配 → 404，**不碰凭据** | 全局 Basic 中间件先行 ⇒ 无凭据 401，且**计入认证失败预算**（10 次/15 分钟封锁 IP） | 官方客户端不发未知路径；风险面是「同 IP 合法客户端 15 分钟内连错 10 次 URL 被锁 15 分钟」（有 `Retry-After`、自动恢复），比上游的裸 404 多一道限速代价 |
| hash 的匹配方式 | `EF.Functions.Like(r.Hash, hash)`（`HistoryService.cs:255`）：`%`/`_` 是通配符，且 SQLite 的 LIKE 对 ASCII 大小写不敏感。**2026-09-15 A/B 实测**：`GET /api/history/Text-<前 8 位>%` → 上游 **200 命中该记录**、本实现 **404** | `LOWER(Hash) = LOWER(?3)`（`src/db.ts:157`） | 本实现更严格、可走索引；第三方客户端发送含 `%`/`_` 的 hash 时，上游会误命中同前缀记录 |
...
| Group zip 的重复条目 | 内容「首次落盘优先」，但 `topLevelFiles`/条目列表**不去重**（`GroupProfile.cs:644-648`）⇒ 重复条目被计入 hash 与 `totalSize` 两次 | 同名条目只取首个，且条目集与顶层条目都**去重**（`src/hash.ts:112-116`、`185-200`） | **有意偏离**：含重复条目的 zip 上两侧 hash 与 size **必然不同**。官方客户端恒不写重复条目，不可达（见 README「协议与功能差异 → 已知限制」的『畸形压缩包的处理口径不同』行） |
...
| `SearchText` 的**字节上限** | 无（D1 的 LIKE 复杂度上限只在**转义后**出现，见下） | 入口按字节校验，超 `MAX_SEARCH_BYTES`（48）→ **400**（`serialization.ts:484-490`） | 有意偏离（fail-loud）：超长模式会在真 D1 上以 `LIKE or GLOB pattern too complex` 500（§95 实测）；官方客户端搜索框把用户原文**直发**（客户端的 `searchText` 字段），**客户端无长度限制** ⇒ 16 个 CJK 字即触发 400（「正常输入远小于此」是假设，非代码事实） |
| `statistics.totalFileSizeMB` 的口径 | `GetStatisticsAsync` 只做 DB 聚合；目录占用由文件系统信息直接取（`docs/design.md` §5） | DB 四个计数同左；`totalFileSizeMB` 来自 **R2 全桶列举累加**（`storage.totalHistorySize`，每页 1000 键） | 值等价、成本不同：每次调用 O(对象数) 次子请求（1 万对象 ≈ 10 次）。单用户规模可忽略；已作为已知成本记入 `design.md` §13 |
...
| `GET /file/{name}` 的**同名候选数** | **无上限**：按 LastAccessed 倒序取全部同名记录，逐条 `File.Exists` 直到命中（`SyncClipboardController.cs:88-99` → `Services/History/HistoryService.cs:212-228`，过滤在 `:226`） | 候选数上限 **32**（`src/db.ts:169` 的 `MAX_TRANSFER_FILE_CANDIDATES`，在 SQL **预筛**的 `LIMIT ?3` 生效而非取回后切片，`:374-376`）⇒ 该请求的 R2 `get` ≤ 32 次；前 32 条的数据都不存在时返回 **404** | **有意偏离**（实现处 `src/routes/webdav.ts` 的候选循环）：Free 的单次调用上限是 1,000 次「到 Cloudflare 服务」的子请求，而每条候选要一次 R2 `get`（上游跑在本地文件系统上、没有这个配额）。正常库同名候选 ≤ 数条 ⇒ 实际不可达；只有「库内同名记录 > 32 且靠前的数据都缺失」时才表现为 404（上游会继续回退到更旧的记录）。**上限之内**的"文件缺失时继续回退到更旧记录"语义逐条不变。**可达面（重要）**：只有把本服务当 **WebDAV 服务器**接入时才会走 `GET /file/{name}`（`PollingDrivenServer` → `StorageBasedServerHelper.DownloadFileAsync`）；**官方服务器模式取数据走 `/api/history/{id}/data`** ⇒ 此偏离在该模式下**结构性不可达** |
